...
class CatchupTest(ProbackupTest, unittest.TestCase):
    ...
    def test_basic_full_catchup(self):
        ...
        """
        # preparation
        src_pg = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "src"), set_replication=True)
        src_pg.slow_start()
        ...
        self.compare_pgdata(self.pgdata_content(src_pg.data_dir), self.pgdata_content(dst_pg.data_dir))
    ...
    def test_full_catchup_with_tablespace(self):
        ...
        src_pg.safe_psql("postgres", "CREATE TABLE ultimate_question TABLESPACE tblspace1 AS SELECT 42 AS answer")
        src_query_result = src_pg.table_checksum("ultimate_question")
        ...
        src_pg.safe_psql("postgres", "UPDATE ultimate_question SET answer = -1")
        src_pg.stop()
    ...
    def test_basic_delta_catchup(self):
        ...
        src_pg.slow_start()
        src_pg.safe_psql("postgres", "CREATE TABLE ultimate_question(answer int)")
    ...
    def test_tli_ptrack_catchup(self):
        ...
        src_pg.safe_psql(
            "postgres", "CHECKPOINT"
        )  # force postgres to update tli in 'SELECT timeline_id FROM pg_catalog.pg_control_checkpoint()'
        src_tli = (
            src_pg.safe_psql("postgres", "SELECT timeline_id FROM pg_catalog.pg_control_checkpoint()")
            .decode("utf-8")
            .rstrip()
        )
    ...
    def test_local_tablespace_without_mapping(self):
        ...
        self.create_tblspace_in_node(src_pg, "tblspace", tblspc_path=tblspace_path)

        src_pg.safe_psql("postgres", "CREATE TABLE ultimate_question TABLESPACE tblspace AS SELECT 42 AS answer")
        ...
        try:
            ...
            self.assertEqual(
                1,
                0,
                "Expecting Error because '-T' parameter is not specified.\n Output: {0} \n CMD: {1}".format(
    ...
    def test_running_dest_postmaster(self):
        ...
        try:
            ...
            self.assertEqual(
                1,
                0,
                "Expecting Error because postmaster in destination is running.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
    ...
    def test_same_db_id(self):
        ...
        try:
            ...
            self.assertEqual(
                1,
                0,
                "Expecting Error because database identifiers mismatch.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
    ...
    def test_unclean_delta_catchup(self):
        ...
        try:
            ...
            self.assertEqual(
                1,
                0,
                "Expecting Error because destination pg is not cleanly shutdowned.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
    ...
    def test_catchup_with_replication_slot(self):
        ...
        try:
            self.catchup_node(
                ...
                source_pgdata=src_pg.data_dir,
                destination_node=dst_pg,
                options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--slot=nonexistentslot_1a"],
            )
            self.assertEqual(
                1,
                0,
                "Expecting Error because replication slot does not exist.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
        ...
        self.catchup_node(
            ...
            source_pgdata=src_pg.data_dir,
            destination_node=dst_pg,
            options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--slot=existentslot_1b"],
        )
        ...
        self.catchup_node(
            ...
            source_pgdata=src_pg.data_dir,
            destination_node=dst_pg,
            options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--slot=nonexistentslot_2a", "--perm-slot"],
        )
        ...
        try:
            self.catchup_node(
                ...
                source_pgdata=src_pg.data_dir,
                destination_node=dst_pg,
                options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--slot=existentslot_2b", "--perm-slot"],
            )
            self.assertEqual(
                1,
                0,
                "Expecting Error because replication slot already exist.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
        ...
        self.catchup_node(
            ...
            source_pgdata=src_pg.data_dir,
            destination_node=dst_pg,
            options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--perm-slot"],
        )
        slot_name = (
            src_pg.safe_psql(
                "postgres",
        ...
        if self.get_version(src_pg) >= self.version_to_num("10.0"):
            ...
            try:
                self.catchup_node(
                    ...
                    source_pgdata=src_pg.data_dir,
                    destination_node=dst_pg,
                    options=["-d", "postgres", "-p", str(src_pg.port), "--stream", "--perm-slot", "--temp-slot"],
                )
                self.assertEqual(
                    1,
                    0,
                    "Expecting Error because conflicting options --perm-slot and --temp-slot used together\n Output: {0} \n CMD: {1}".format(
                        repr(self.output), self.cmd
                    ),
                )
            except ProbackupException as e:
    ...
    def test_catchup_with_exclude_path(self):
        ...
        self.catchup_node(
            ...
            options=[
                ...
                "-x",
                "{0}".format(os.path.join(src_pg.data_dir, "src_garbage_dir")),
            ],
        )
    ...
    def test_dry_run_catchup_full(self):
        ...
        self.compare_pgdata(content_before, self.pgdata_content(dst_pg.data_dir))
    ...
    def test_pgdata_is_ignored(self):
        ...
        self.compare_pgdata(self.pgdata_content(node.data_dir), self.pgdata_content(dest.data_dir))
        ...
        self.compare_pgdata(self.pgdata_content(node.data_dir), self.pgdata_content(dest2.data_dir))
