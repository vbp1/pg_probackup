...
class CrossCompatibilityTest(ProbackupTest, unittest.TestCase):
    ...
    def test_catchup_with_different_remote_major_pg(self):
        ...
        src_pg.safe_psql("postgres", "CREATE TABLE ultimate_question2 AS SELECT 42 AS answer")
        ...
        self.catchup_node(
            ...
            options=[
                ...
                str(src_pg.port),
                "--stream",
                "--remote-path=" + pgprobackup_ssh_agent_path,
            ],
        )
...
class CompatibilityTest(ProbackupTest, unittest.TestCase):
    ...
    def test_backward_compatibility_page(self):
        ...
        pgbench = node.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "4", "-T", "20"])
        pgbench.wait()
        pgbench.stdout.close()

        self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)
        ...
        node.safe_psql("postgres", "create table tmp as select * from pgbench_accounts where aid < 1000")

        node.safe_psql("postgres", "delete from pgbench_accounts")
        ...
        node.safe_psql("postgres", "insert into pgbench_accounts select * from pgbench_accounts")
    ...
    def test_backward_compatibility_delta(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="delta", old_binary=True)
    ...
    def test_backward_compatibility_ptrack(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="ptrack", old_binary=True)
        ...
        self.restore_node(
            ...
            "node",
            node_restored,
            options=["-j", "4", "--recovery-target=latest", "--recovery-target-action=promote"],
        )
    ...
    def test_backward_compatibility_compression(self):
        ...
        # FULL backup with OLD binary
        backup_id = self.backup_node(backup_dir, "node", node, old_binary=True, options=["--compress"])
        ...
        # PAGE backup with OLD binary
        pgbench = node.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "4", "-T", "10"])
        pgbench.wait()
        pgbench.stdout.close()

        self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])
        ...
        self.backup_node(backup_dir, "node", node, old_binary=True, options=["--compress"])
        ...
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--compress"], old_binary=True)
        ...
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--compress"])
    ...
    def test_backward_compatibility_merge(self):
        ...
        # PAGE backup with OLD binary
        backup_id = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)
    ...
    def test_backward_compatibility_merge_1(self):
        ...
        pgbench = node.pgbench(
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "1", "-T", "10", "--no-vacuum"]
        )
        pgbench.wait()
        pgbench.stdout.close()
        ...
        self.assertIn("WARNING: In-place merge is disabled because of storage format incompatibility", output)
    ...
    def test_backward_compatibility_merge_2(self):
        ...
        # PAGE1 backup with OLD binary
        page1 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)

        pgdata1 = self.pgdata_content(node.data_dir)

        node.safe_psql("postgres", "DELETE from pgbench_accounts where ctid > '(10,1)'")

        # PAGE2 backup with OLD binary
        page2 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)

        pgdata2 = self.pgdata_content(node.data_dir)

        # PAGE3 backup with OLD binary
        page3 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)
        ...
        page4 = self.backup_node(backup_dir, "node", node, backup_type="page")
        pgdata4 = self.pgdata_content(node.data_dir)

        # merge backups one by one and check data correctness
        # merge PAGE1
        self.merge_backup(backup_dir, "node", page1, options=["--log-level-file=VERBOSE"])

        # check data correctness for PAGE1
        node_restored.cleanup()
        self.restore_node(backup_dir, "node", node_restored, backup_id=page1, options=["--log-level-file=VERBOSE"])
        pgdata_restored = self.pgdata_content(node_restored.data_dir)
    ...
    def test_backward_compatibility_merge_3(self):
        ...
        # PAGE1 backup with OLD binary
        page1 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])
        ...
        # PAGE2 backup with OLD binary
        page2 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])

        pgdata2 = self.pgdata_content(node.data_dir)

        # PAGE3 backup with OLD binary
        page3 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])
        ...
        # PAGE4 backup with NEW binary
        page4 = self.backup_node(backup_dir, "node", node, backup_type="page", options=["--compress"])
        pgdata4 = self.pgdata_content(node.data_dir)
    ...
    def test_backward_compatibility_merge_4(self):
        ...
        if self.version_to_num(self.old_probackup_version) > self.version_to_num("2.4.0"):
            self.assertTrue(False, "You need pg_probackup old_binary =< 2.4.0 for this test")
        ...
        self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored"))

        # FULL backup with OLD binary
        self.backup_node(backup_dir, "node", node, old_binary=True, options=["--compress"])

        pgbench = node.pgbench(
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "1", "-T", "20", "--no-vacuum"]
        )
        pgbench.wait()
        pgbench.stdout.close()

        # PAGE backup with NEW binary
        page_id = self.backup_node(backup_dir, "node", node, backup_type="page", options=["--compress"])
    ...
    def test_backward_compatibility_merge_5(self):
        ...
        self.assertNotEqual(
            self.version_to_num(self.old_probackup_version), self.version_to_num(self.probackup_version)
        )
        ...
        # check that in-place is disabled
        self.assertNotIn("WARNING: In-place merge is disabled because of storage format incompatibility", output)
    ...
    def test_page_vacuum_truncate(self):
        ...
        id2 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)
        pgdata2 = self.pgdata_content(node.data_dir)
        ...
        id3 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True)
        pgdata3 = self.pgdata_content(node.data_dir)

        node_restored = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored"))
        node_restored.cleanup()

        self.restore_node(backup_dir, "node", node_restored, data_dir=node_restored.data_dir, backup_id=id1)
        ...
        self.restore_node(backup_dir, "node", node_restored, data_dir=node_restored.data_dir, backup_id=id2)
        ...
        self.restore_node(backup_dir, "node", node_restored, data_dir=node_restored.data_dir, backup_id=id3)
    ...
    def test_page_vacuum_truncate_compressed_1(self):
        ...
        id1 = self.backup_node(backup_dir, "node", node, old_binary=True, options=["--compress"])
        pgdata1 = self.pgdata_content(node.data_dir)
        ...
        id2 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])
        pgdata2 = self.pgdata_content(node.data_dir)
        ...
        id3 = self.backup_node(backup_dir, "node", node, backup_type="page", old_binary=True, options=["--compress"])
        pgdata3 = self.pgdata_content(node.data_dir)
    ...
    def test_compatibility_tablespace(self):
        ...
        backup_id = self.backup_node(
            backup_dir, "node", node, backup_type="full", options=["-j", "4", "--stream"], old_binary=True
        )

        tblspace_old_path = self.get_tblspace_path(node, "tblspace_old")

        self.create_tblspace_in_node(node, "tblspace", tblspc_path=tblspace_old_path)
        ...
        try:
            self.restore_node(
                ...
                "node",
                node_restored,
                options=["-j", "4", "-T", "{0}={1}".format(tblspace_old_path, tblspace_new_path)],
            )
        ...
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream"], old_binary=True
        )
