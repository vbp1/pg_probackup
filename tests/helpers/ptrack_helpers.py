...
idx_ptrack = {
    "t_heap": {"type": "heap"},
    "t_btree": {"type": "btree", "column": "text", "relation": "t_heap"},
    "t_seq": {"type": "seq", "column": "t_seq", "relation": "t_heap"},
    "t_spgist": {"type": "spgist", "column": "text", "relation": "t_heap"},
    "t_brin": {"type": "brin", "column": "text", "relation": "t_heap"},
    "t_gist": {"type": "gist", "column": "tsvector", "relation": "t_heap"},
    "t_gin": {"type": "gin", "column": "tsvector", "relation": "t_heap"},
    "t_hash": {"type": "hash", "column": "id", "relation": "t_heap"},
    "t_bloom": {"type": "bloom", "column": "id", "relation": "t_heap"},
}
...
class ProbackupTest:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gdb = "PGPROBACKUP_GDB" in self.test_env and self.test_env["PGPROBACKUP_GDB"] == "ON"

        self.paranoia = "PG_PROBACKUP_PARANOIA" in self.test_env and self.test_env["PG_PROBACKUP_PARANOIA"] == "ON"

        self.archive_compress = "ARCHIVE_COMPRESSION" in self.test_env and self.test_env["ARCHIVE_COMPRESSION"] == "ON"
        ...
        if not self.probackup_path:
            probackup_path_tmp = os.path.join(testgres.get_pg_config()["BINDIR"], "pg_probackup")

            if os.path.isfile(probackup_path_tmp):
                if not os.access(probackup_path_tmp, os.X_OK):
                    print("{0} is not an executable file".format(probackup_path_tmp))
                else:
                    self.probackup_path = probackup_path_tmp

        if not self.probackup_path:
            probackup_path_tmp = os.path.abspath(os.path.join(self.dir_path, "../pg_probackup"))
        ...
        if os.name == "posix":
            ...
            os.environ["PATH"] = os.path.dirname(self.probackup_path) + ":" + os.environ["PATH"]
        ...
        if "PGPROBACKUPBIN_OLD" in self.test_env:
            if os.path.isfile(self.test_env["PGPROBACKUPBIN_OLD"]) and os.access(
                self.test_env["PGPROBACKUPBIN_OLD"], os.X_OK
            ):
    ...
    def is_test_result_ok(test_case):
        ...
        ok = ok and all(getattr(test, "test_case", None) != test_case for test, text in result.errors + result.failures)
    ...
    def pg_config_version(self):
        return self.version_to_num(testgres.get_pg_config()["VERSION"].split(" ")[1])
    ...
    def make_empty_node(self, base_dir=None, port=None, bin_dir=None):
    ...
    def make_simple_node(
        ...
        if node.major_version >= 13:
            self.set_auto_conf(node, {}, "postgresql.conf", ["wal_keep_segments"])

        return node

    def simple_bootstrap(self, node, role) -> None:
        node.safe_psql("postgres", "CREATE ROLE {0} WITH LOGIN REPLICATION".format(role))
    ...
    def create_tblspace_in_node(self, node, tblspc_name, tblspc_path=None, cfs=False):
        res = node.execute(
            "postgres", "select exists (select 1 from pg_tablespace where spcname = '{0}')".format(tblspc_name)
        )
    ...
    def drop_tblspace(self, node, tblspc_name):
        ...
        # Check that tablespace with name 'tblspc_name' do not exists already
        self.assertTrue(res[0][0], 'Tablespace "{0}" do not exists'.format(tblspc_name))
        ...
        for rel in rels:
            node.safe_psql("postgres", "DROP TABLE {0}".format(rel[0]))

        node.safe_psql("postgres", "DROP TABLESPACE {0}".format(tblspc_name))
    ...
    def get_fork_size(self, node, fork_name):
        return node.execute("postgres", "select pg_relation_size('{0}')/8192".format(fork_name))[0][0]
    ...
    def check_ptrack_map_sanity(self, node, idx_ptrack):
        ...
        self.assertTrue(success, "Ptrack has failed to register changes in data files")
        ...
        for PageNum in range(size):
                    ...
                    if self.verbose:
                        print(
                            ...
                            " but ptrack value is {3}. THIS IS BAD".format(
                                idx_dict["path"], PageNum, idx_dict["type"], idx_dict["ptrack"][PageNum]
                            )
                        )
                        # print(idx_dict)
                    success = False
            ...
            if idx_dict["new_pages"][PageNum] != idx_dict["old_pages"][PageNum]:
                    ...
                    if self.verbose:
                        ...
                        print(
                            "  Old checksumm: {0}\n  New checksumm: {1}".format(
                                idx_dict["old_pages"][PageNum], idx_dict["new_pages"][PageNum]
                            )
                        )
                ...
                if idx_dict["ptrack"][PageNum] != 0:
                    if self.verbose:
                        print(
                            "File: {0}\n Page Number {1} of type {2} was not changed, but ptrack value is {3}".format(
                                idx_dict["path"], PageNum, idx_dict["type"], idx_dict["ptrack"][PageNum]
                            )
                        )
            return success
    ...
    def get_backup_filelist(self, backup_dir, instance, backup_id):
        filelist_path = os.path.join(backup_dir, "backups", instance, backup_id, "backup_content.control")
        ...
        try:
            ...
            if asynchronous:
                return subprocess.Popen(
                    [binary_path] + command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env
                )
            else:
                self.output = subprocess.check_output(
                    [binary_path] + command, stderr=subprocess.STDOUT, env=env
                ).decode("utf-8")
    ...
    def init_pb(self, backup_dir, options=None, old_binary=False):
        ...
        if self.remote and not old_binary:
            options = options + ["--remote-proto=ssh", "--remote-host=localhost"]

        return self.run_pb(["init", "-B", backup_dir] + options, old_binary=old_binary)

    def add_instance(self, backup_dir, instance, node, old_binary=False, options=None):
        if options is None:
            options = []
        cmd = ["add-instance", "--instance={0}".format(instance), "-B", backup_dir, "-D", node.data_dir]
    ...
    def set_config(self, backup_dir, instance, old_binary=False, options=None):
        if options is None:
            options = []
        cmd = [
            "set-config",
    ...
    def set_backup(self, backup_dir, instance, backup_id=False, old_binary=False, options=None):
        if options is None:
            options = []
        cmd = ["set-backup", "-B", backup_dir]
    ...
    def del_instance(self, backup_dir, instance, old_binary=False):
        return self.run_pb(["del-instance", "--instance={0}".format(instance), "-B", backup_dir], old_binary=old_binary)
    ...
    def merge_backup(
        self, backup_dir, instance, backup_id, asynchronous=False, gdb=False, old_binary=False, options=None
    ):
        if options is None:
            options = []
        cmd_list = ["merge", "-B", backup_dir, "--instance={0}".format(instance), "-i", backup_id]

        return self.run_pb(cmd_list + options, asynchronous, gdb, old_binary)

    def restore_node(
        self, backup_dir, instance, node=False, data_dir=None, backup_id=None, old_binary=False, options=None, gdb=False
    ):
        if options is None:
            options = []
        if data_dir is None:
        ...
        cmd_list = ["restore", "-B", backup_dir, "-D", data_dir, "--instance={0}".format(instance)]
    ...
    def catchup_node(self, backup_mode, source_pgdata, destination_node, options=None):
        if options is None:
            options = []
        cmd_list = [
            "catchup",
        ...
        if self.verbose:
            cmd_list += ["--log-level-file=VERBOSE", "--log-directory={0}".format(destination_node.logs_dir)]
    ...
    def show_pb(
        ...
        if as_json:
            ...
            show_splitted = self.run_pb(cmd_list + options, old_binary=old_binary, env=env).splitlines()
            if instance is not None and backup_id is None:
                ...
                for backup_record in body:
                    ...
                    if len(header_split) != len(backup_record_split):
                        print(
                            warning.format(
                                header=header, body=body, header_split=header_split, body_split=backup_record_split
                            )
                        )
                        exit(1)
    ...
    def show_archive(self, backup_dir, instance=None, options=None, as_text=False, as_json=True, old_binary=False, tli=0):
        ...
        if as_json:
            ...
            show_splitted = self.run_pb(cmd_list + options, old_binary=old_binary).splitlines()
            print(show_splitted)
            exit(1)

    def validate_pb(
        self, backup_dir, instance=None, backup_id=None, options=None, old_binary=False, gdb=False, asynchronous=False
    ):
        if options is None:
            options = []
        cmd_list = ["validate", "-B", backup_dir]
        if instance:
            cmd_list += ["--instance={0}".format(instance)]
        if backup_id:
            cmd_list += ["-i", backup_id]
    ...
    def delete_pb(
        self, backup_dir, instance, backup_id=None, options=None, old_binary=False, gdb=False, asynchronous=False
    ):
        if options is None:
            options = []
        cmd_list = ["delete", "-B", backup_dir]
    ...
    def delete_expired(self, backup_dir, instance, options=None, old_binary=False):
        if options is None:
            options = []
        cmd_list = ["delete", "-B", backup_dir, "--instance={0}".format(instance)]
        return self.run_pb(cmd_list + options, old_binary=old_binary)

    def show_config(self, backup_dir, instance, old_binary=False):
        out_dict = {}
        cmd_list = ["show-config", "-B", backup_dir, "--instance={0}".format(instance)]
    ...
    def get_recovery_conf(self, node):
        ...
        with open(recovery_conf_path) as recovery_conf:
    ...
    def set_archiving(
        ...
        if custom_archive_command is None:
            if os.name == "posix":
                ...
                options["archive_command"] = '"{0}" archive-push -B {1} --instance={2} '.format(
                    self.probackup_path.replace("\\", "\\\\"), backup_dir.replace("\\", "\\\\"), instance
                )
            ...
            if archive_timeout:
                options["archive_command"] += "--archive-timeout={0} ".format(archive_timeout)
    ...
    def set_auto_conf(self, node, options, config="postgresql.auto.conf", rm_options=None):
        # parse postgresql.auto.conf
        ...
        for option in current_options:
            auto_conf += "{0} = '{1}'\n".format(option, current_options[option])
    ...
    def set_replica(self, master, replica, replica_name="replica", synchronous=False, log_shipping=False):
        self.set_auto_conf(replica, options={"port": replica.port, "hot_standby": "on"})
            ...
            if not log_shipping:
                self.set_auto_conf(
                    replica,
                    {
                        ...
                        " sslmode=prefer sslcompression=1".format(self.user, master.port, replica_name)
                    },
                    config,
                )
        else:
        ...
        if synchronous:
            self.set_auto_conf(
                master, options={"synchronous_standby_names": replica_name, "synchronous_commit": "remote_apply"}
            )

            master.reload()

    def change_backup_status(self, backup_dir, instance, backup_id, status):
        control_file_path = os.path.join(backup_dir, "backups", instance, backup_id, "backup.control")
    ...
    def wait_until_replica_catch_with_master(self, master, replica):
        version = master.safe_psql("postgres", "show server_version").decode("utf-8").rstrip()
        ...
        lsn = master.safe_psql("postgres", "SELECT {0}".format(master_function)).decode("utf-8").rstrip()

        # Wait until replica catch up with master
        replica.poll_query_until("postgres", "SELECT '{0}'::pg_lsn <= {1}".format(lsn, replica_function))
    ...
    def get_ptrack_version(self, node):
        version = (
            node.safe_psql("postgres", "SELECT extversion FROM pg_catalog.pg_extension WHERE extname = 'ptrack'")
            .decode("utf-8")
            .rstrip()
        )
    ...
    def del_test_dir(self, module_name, fname):
        ...
        shutil.rmtree(os.path.join(self.tmp_path, module_name, fname), ignore_errors=True)
    ...
    def pgdata_content(self, pgdata, ignore_ptrack=True, exclude_dirs=None):
        ...
        for root, dirs, files in os.walk(pgdata, followlinks=True):
            ...
            for file in files:
                if file in files_to_ignore or (ignore_ptrack and file.endswith("_ptrack")):
                    continue
                ...
                if cfile.is_datafile:
                    ...
                    cfile.md5_per_page = self.get_md5_per_page_for_fork(file_fullpath, size_in_pages)
    ...
    def get_known_bugs_comparision_exclusion_dict(self, node):
        ...
        for filename in spgist_filelist:
            comparision_exclusion_dict[filename] = {0}

        return comparision_exclusion_dict

    def compare_pgdata(self, original_pgdata, restored_pgdata, exclusion_dict=None):
        """
        ...
        for directory in sorted(restored_dirs - original_dirs):
            ...
            error_message += " in original PGDATA: {0}\n".format(os.path.join(restored_pgdata["pgdata"], directory))
        ...
        for directory in sorted(original_dirs & restored_dirs):
            ...
            if original.mode != restored.mode:
                ...
                error_message += " Dir old: {0} Permissions: {1}\n".format(
                    os.path.join(original_pgdata["pgdata"], directory), original.mode
                )
                error_message += " Dir new: {0} Permissions: {1}\n".format(
                    os.path.join(restored_pgdata["pgdata"], directory), restored.mode
                )
        ...
        for file in sorted(restored_files - original_files):
            ...
            fail = True
            error_message += "\nFile is not present"
            error_message += " in original PGDATA: {0}\n".format(os.path.join(restored_pgdata["pgdata"], file))
        ...
        for file in sorted(original_files & restored_files):
            ...
            if restored.mode != original.mode:
                ...
                error_message += " File_old: {0} Permissions: {1:o}\n".format(
                    os.path.join(original_pgdata["pgdata"], file), original.mode
                )
                error_message += " File_new: {0} Permissions: {1:o}\n".format(
                    os.path.join(restored_pgdata["pgdata"], file), restored.mode
                )
            ...
            if original.md5 != restored.md5:
                ...
                for page in sorted(original_pages - restored_pages):
                    error_message += "\n Page {0} dissappeared.\n File: {1}\n".format(
                        page, os.path.join(restored_pgdata["pgdata"], file)
                    )
    ...
    def _check_gdb_flag_or_skip_test(self):
        if not self.gdb:
            self.skipTest("Specify PGPROBACKUP_GDB and build without optimizations for run this test")
...
class GDBobj:
    def __init__(self, cmd, env, attach=False):
        ...
        # Get version
        gdb_version_number = re.search(rb"^GNU gdb [^\d]*(\d+)\.(\d)", gdb_version)
    ...
    def remove_all_breakpoints(self):
        ...
        raise GdbException("Failed to remove breakpoints.\n Output:\n {0}".format(result))
    ...
    def run_until_break(self):
        ...
        for line in result:
            if line.startswith('*stopped,reason="breakpoint-hit"'):
                return
        raise GdbException("Failed to run until breakpoint.\n")
    ...
    def continue_execution_until_running(self):
        ...
        raise GdbException("Failed to continue execution until running.\n")
    ...
    def continue_execution_until_exit(self):
        ...
        raise GdbException("Failed to continue execution until exit.\n")
    ...
    def continue_execution_until_error(self):
        ...
        for line in result:
            ...
            if line.startswith('*stopped,reason="exited'):
                return
            if line.startswith('*stopped,reason="signal-received",signal-name="SIGABRT"'):
                return

        raise GdbException("Failed to continue execution until error.\n")

    def continue_execution_until_break(self, ignore_count=0):
        if ignore_count > 0:
            result = self._execute("continue " + str(ignore_count), False)
        else:
            result = self._execute("continue", False)
