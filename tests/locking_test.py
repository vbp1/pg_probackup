...
class LockingTest(ProbackupTest, unittest.TestCase):
    ...
    def test_locking_running_validate_1(self):
        ...
        self.assertEqual("RUNNING", self.show_pb(backup_dir, "node")[1]["status"])

        validate_output = self.validate_pb(backup_dir, options=["--log-level-console=LOG"])
    ...
    def test_locking_running_validate_2(self):
        ...
        try:
            ...
            self.assertTrue(
                "which used backup {0} no longer exists".format(backup_id) in e.message
                and "Backup {0} has status RUNNING, change it to ERROR and skip validation".format(backup_id)
                in e.message
        ...
        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[1]["status"])
    ...
    def test_locking_running_validate_2_specific_id(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "Backup {0} has status RUNNING, change it to ERROR and skip validation".format(backup_id)
                in e.message
                and "ERROR: Backup {0} has status: ERROR".format(backup_id) in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        try:
            ...
            self.assertTrue(
                "WARNING: Backup {0} has status ERROR. Skip validation".format(backup_id) in e.message
                and "WARNING: Some backups are not valid" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_locking_running_3(self):
        ...
        os.remove(os.path.join(backup_dir, "backups", "node", backup_id, "backup.pid"))
        ...
        try:
            ...
            self.assertTrue(
                "Backup {0} has status RUNNING, change it to ERROR and skip validation".format(backup_id) in e.message
                and "WARNING: Some backups are not valid" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_locking_restore_locked(self):
        ...
        gdb = self.validate_pb(backup_dir, "node", backup_id=backup_id, gdb=True)
    ...
    def test_concurrent_delete_and_restore(self):
        ...
        gdb = self.delete_pb(backup_dir, "node", backup_id=backup_id, gdb=True)
        ...
        try:
            ...
            self.assertTrue(
                "Backup {0} is used without validation".format(restore_id) in e.message
                and "is using backup {0}, and is still running".format(backup_id) in e.message
                and "ERROR: Cannot lock backup" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_backup_directory_name(self):
        ...
        self.set_backup(backup_dir, "node", full_id_2, options=["--note=hello"])

        self.merge_backup(backup_dir, "node", page_id_2, options=["-j", "4"])

        self.assertNotIn("note", self.show_pb(backup_dir, "node", page_id_2))
    ...
    def test_empty_lock_file(self):
        ...
        self.assertIn("Waiting 30 seconds on empty exclusive lock for backup", out)
    ...
    def test_shared_lock(self):
        ...
        self.assertFalse(os.path.exists(lockfile_excl), "File should not exist: {0}".format(lockfile_excl))

        self.assertFalse(os.path.exists(lockfile_shr), "File should not exist: {0}".format(lockfile_shr))
