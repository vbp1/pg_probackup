...
class ExcludeTest(ProbackupTest, unittest.TestCase):
    ...
    def test_exclude_temp_files(self):
        ...
        node = self.make_simple_node(
            ...
            pg_options={"logging_collector": "on", "log_filename": "postgresql.log"},
        )
        ...
        node.safe_psql("postgres", "select oid from pg_database where datname = 'postgres'").rstrip()
        ...
        full_id = self.backup_node(backup_dir, "node", node, backup_type="full", options=["--stream"])

        file = os.path.join(backup_dir, "backups", "node", full_id, "database", "base", "pgsql_tmp", "pgsql_tmp7351.16")

        self.assertFalse(os.path.exists(file), "File must be excluded: {0}".format(file))
    ...
    def test_exclude_temp_tables(self):
        ...
        with node.connect("postgres") as conn:
            ...
            toast_path = conn.execute(
                "select pg_relation_filepath('{0}.{1}')".format(temp_toast_schema_name, "pg_toast_" + str(heap_oid))
            )[0][0]
            conn.commit()

            toast_idx_path = conn.execute(
                "select pg_relation_filepath('{0}.{1}')".format(
                    temp_toast_schema_name, "pg_toast_" + str(heap_oid) + "_index"
                )
            )[0][0]
            conn.commit()
        ...
        for _root, _dirs, files in os.walk(backup_dir):
            for file in files:
                if file in [
                    temp_table_filename,
                    temp_table_filename + ".1",
                    temp_idx_filename,
                    temp_idx_filename + ".1",
                    temp_toast_filename,
    ...
    def test_exclude_unlogged_tables_1(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"shared_buffers": "10MB"},
        )
    ...
    def test_exclude_unlogged_tables_2(self):
        ...
        for backup_type in ["full", "delta", "page"]:
            if backup_type == "full":
                node.safe_psql("postgres", "create unlogged table test as select generate_series(0,20050000)::text")
            else:
                node.safe_psql("postgres", "insert into test select generate_series(0,20050000)::text")

            rel_path = node.execute("postgres", "select pg_relation_filepath('test')")[0][0]

            backup_id = self.backup_node(backup_dir, "node", node, backup_type=backup_type, options=["--stream"])

            backup_ids.append(backup_id)

            filelist = self.get_backup_filelist(backup_dir, "node", backup_id)

            self.assertNotIn(rel_path, filelist, "Unlogged table was not excluded")

            self.assertNotIn(rel_path + ".1", filelist, "Unlogged table was not excluded")

            self.assertNotIn(rel_path + ".2", filelist, "Unlogged table was not excluded")

            self.assertNotIn(rel_path + ".3", filelist, "Unlogged table was not excluded")
        ...
        for backup_id in backup_ids:
            ...
            self.assertEqual(node.execute("postgres", "select count(*) from test")[0][0], 0)
    ...
    def test_exclude_log_dir(self):
        ...
        log_dir = node.safe_psql("postgres", "show log_directory").decode("utf-8").rstrip()
    ...
    def test_exclude_log_dir_1(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="full", options=["--stream", "--backup-pg-log"])
