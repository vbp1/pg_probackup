...
class DeltaTest(ProbackupTest, unittest.TestCase):
    ...
    def test_delta_vacuum_truncate_1(self):
        ...
        self.restore_node(
            backup_dir, "node", node_restored, options=["-T", "{0}={1}".format(old_tablespace, new_tablespace)]
        )
    ...
    def test_delta_vacuum_truncate_2(self):
        ...
        filepath = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
    ...
    def test_delta_stream(self):
        ...
        delta_backup_id = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--stream"])
    ...
    def test_delta_archive(self):
        ...
        delta_result = node.table_checksum("t_heap")
        delta_backup_id = self.backup_node(backup_dir, "node", node, backup_type="delta")
    ...
    def test_delta_multiple_segments(self):
        ...
        # CREATE TABLE
        node.pgbench_init(scale=100, options=["--tablespace=somedata", "--no-vacuum"])
        # FULL BACKUP
        self.backup_node(backup_dir, "node", node, options=["--stream"])
    ...
    def test_exists_in_previous_backup(self):
        ...
        fullpath = os.path.join(backup_dir, "backups", "node", backup_id, "database", filepath)
    ...
    def test_alter_database_set_tablespace_delta(self):
        ...
        # ALTER TABLESPACE
        self.create_tblspace_in_node(node, "somedata_new")
        node.safe_psql("postgres", "alter database db1 set tablespace somedata_new")
    ...
    def test_delta_nullified_heap_page_backup(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--log-level-file=verbose"])
        ...
        if not self.remote:
            ...
            self.assertIn('VERBOSE: File: "{0}" blknum 1, empty page'.format(file), content)
            self.assertNotIn("Skipping blknum 1 in file: {0}".format(file), content)
    ...
    def test_delta_backup_from_past(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "of previous backup " in e.message
