...
class MergeTest(ProbackupTest, unittest.TestCase):
    def test_basic_merge_full_page(self):
        ...
        with node.connect() as conn:
            ...
            conn.execute("insert into test select i from generate_series(1,10) s(i)")
            conn.commit()
        ...
        # Merge all backups
        self.merge_backup(backup_dir, "node", page_id, options=["-j", "4"])
    ...
    def test_merge_compressed_backups_1(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--compress", "--stream"])
    ...
    def test_merge_compressed_and_uncompressed_backups(self):
        ...
        # Do compressed FULL backup
        self.backup_node(backup_dir, "node", node, options=["--compress-algorithm=zlib", "--stream"])
        show_backup = self.show_pb(backup_dir, "node")[0]
    ...
    def test_merge_compressed_and_uncompressed_backups_1(self):
        ...
        # Do compressed PAGE backup
        self.backup_node(backup_dir, "node", node, backup_type="page", options=["--compress-algorithm=zlib"])
    ...
    def test_merge_compressed_and_uncompressed_backups_2(self):
        ...
        # Do compressed DELTA backup
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--compress-algorithm=zlib", "--stream"]
        )
    ...
    def test_merge_tablespaces(self):
        ...
        # Drop old tablespace
        node.safe_psql("postgres", "drop tablespace somedata")
        ...
        node.stop()
        shutil.rmtree(self.get_tblspace_path(node, "somedata"), ignore_errors=True)
        shutil.rmtree(self.get_tblspace_path(node, "somedata1"), ignore_errors=True)
        node.cleanup()
    ...
    def test_merge_page_truncate(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "300s"},
        )
        ...
        node.safe_psql("postgres", "delete from t_heap where ctid >= '(11,0)'")
        node.safe_psql("postgres", "vacuum t_heap")
        ...
        self.restore_node(
            ...
            "node",
            node_restored,
            options=["-j", "4", "-T", "{0}={1}".format(old_tablespace, new_tablespace)],
        )
    ...
    def test_merge_ptrack_truncate(self):
        ...
        page_id = self.backup_node(backup_dir, "node", node, backup_type="ptrack")
    ...
    def test_merge_delta_delete(self):
        ...
        node.safe_psql("postgres", "delete from t_heap")
        ...
        self.restore_node(
            ...
            options=[
                ...
                "-T",
                "{0}={1}".format(
                    self.get_tblspace_path(node, "somedata"), self.get_tblspace_path(node_restored, "somedata")
                ),
            ],
        )
    ...
    def test_continue_failed_merge_with_corrupted_delta_backup(self):
        ...
        old_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()

        # DELTA BACKUP
        self.backup_node(backup_dir, "node", node, backup_type="delta")

        node.safe_psql("postgres", "update t_heap set id = 100500")

        node.safe_psql("postgres", "vacuum full t_heap")

        new_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()

        # DELTA BACKUP
        backup_id_2 = self.backup_node(backup_dir, "node", node, backup_type="delta")
        ...
        file = os.path.join(backup_dir, "backups", "node", backup_id_2, "database", new_path)
        ...
        file = os.path.join(backup_dir, "backups", "node", backup_id, "database", old_path)
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: Backup {0} has status CORRUPT, merge is aborted".format(backup_id) in e.message,
    ...
    def test_continue_failed_merge_3(self):
        ...
        # DELETE PAGE1
        shutil.rmtree(os.path.join(backup_dir, "backups", "node", backup_id_delete))
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: Incremental chain is broken, merge is impossible to finish" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_merge_different_compression_algo(self):
        ...
        # FULL backup
        self.backup_node(backup_dir, "node", node, options=["--compress-algorithm=zlib"])
        ...
        # DELTA BACKUP
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--compress-algorithm=pglz"])
    ...
    def test_merge_different_wal_modes(self):
        ...
        self.assertEqual("ARCHIVE", self.show_pb(backup_dir, "node", backup_id)["wal"])
        ...
        self.assertEqual("STREAM", self.show_pb(backup_dir, "node", backup_id)["wal"])
    ...
    def test_crash_after_opening_backup_control_1(self):
        ...
        self.assertEqual("MERGING", self.show_pb(backup_dir, "node")[0]["status"])

        self.assertEqual("MERGING", self.show_pb(backup_dir, "node")[1]["status"])
    ...
    def test_crash_after_opening_backup_control_2(self):
        ...
        path = node.safe_psql("postgres", "select pg_relation_filepath('pgbench_accounts')").decode("utf-8").rstrip()

        fsm_path = path + "_fsm"

        node.safe_psql("postgres", "vacuum pgbench_accounts")
        ...
        file_to_remove = os.path.join(backup_dir, "backups", "node", full_id, "database", fsm_path)
    ...
    def test_losing_file_after_failed_merge(self):
        ...
        # Change data
        node.safe_psql("postgres", "update pgbench_accounts set aid = aid + 1005000")
        ...
        # In to_backup drop file that comes from from_backup
        # emulate crash during previous merge
        file_to_remove = os.path.join(backup_dir, "backups", "node", full_id, "database", vm_path)
        ...
        self.assertEqual("OK", self.show_pb(backup_dir, "node")[0]["status"])
    ...
    def test_failed_merge_after_delete(self):
        ...
        dboid = (
            node.safe_psql("postgres", "select oid from pg_database where datname = 'testdb'").decode("utf-8").rstrip()
        )
        ...
        gdb = self.merge_backup(backup_dir, "node", page_id, gdb=True, options=["--log-level-console=verbose"])
        ...
        # backup half-merged
        self.assertEqual("MERGED", self.show_pb(backup_dir, "node")[0]["status"])

        self.assertEqual(full_id, self.show_pb(backup_dir, "node")[0]["id"])

        os.path.join(backup_dir, "backups", "node", full_id, "database", "base", dboid)

        try:
            self.merge_backup(backup_dir, "node", page_id_2, options=["--log-level-console=verbose"])
            self.assertEqual(
                1,
                0,
            ...
            self.assertTrue(
                "ERROR: Full backup {0} has unfinished merge with backup {1}".format(full_id, page_id) in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_failed_merge_after_delete_1(self):
        ...
        page_1 = self.backup_node(backup_dir, "node", node, backup_type="page")
    ...
    def test_failed_merge_after_delete_2(self):
        ...
        # take PAGE2 backup
        page_2 = self.backup_node(backup_dir, "node", node, backup_type="page")

        gdb = self.merge_backup(backup_dir, "node", page_2, gdb=True, options=["--log-level-console=VERBOSE"])
        ...
        try:
            ...
            self.assertIn(
                "ERROR: Full backup {0} has unfinished merge with backup {1}".format(full_id, page_2),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_failed_merge_after_delete_3(self):
        ...
        node.safe_psql("postgres", "select oid from pg_database where datname = 'testdb'").rstrip()
        ...
        db_path = os.path.join(backup_dir, "backups", "node", full_id)
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: Failed to find parent full backup for {0}".format(page_id_2) in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_merge_backup_from_future(self):
        ...
        self.restore_node(backup_dir, "node", node_restored, backup_id=backup_id)
    ...
    def test_merge_multiple_descendants(self):
        ...
        self.merge_backup(backup_dir, "node", page_id_a2, options=["--merge-expired", "--log-level-console=log"])

        try:
            self.merge_backup(backup_dir, "node", page_id_a3, options=["--merge-expired", "--log-level-console=log"])
            self.assertEqual(
                1,
                0,
            ...
            self.assertTrue(
                "ERROR: Failed to find parent full backup for {0}".format(page_id_a3) in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_smart_merge(self):
        ...
        self.get_backup_filelist_diff(filelist_full, filelist_page)

        # merge PAGE backup
        self.merge_backup(backup_dir, "node", page_id, options=["--log-level-file=VERBOSE"])
    ...
    def test_idempotent_merge(self):
        ...
        gdb = self.merge_backup(backup_dir, "node", page_id_2, gdb=True, options=["--log-level-console=verbose"])
        ...
        self.assertEqual(page_id_2, self.show_pb(backup_dir, "node")[0]["id"])
    ...
    def test_merge_correct_inheritance(self):
        ...
        self.set_backup(backup_dir, "node", page_id, options=["--note=hello", "--ttl=20d"])
        ...
        self.assertEqual(page_meta["note"], self.show_pb(backup_dir, "node", page_id)["note"])

        self.assertEqual(page_meta["expire-time"], self.show_pb(backup_dir, "node", page_id)["expire-time"])
    ...
    def test_merge_correct_inheritance_1(self):
        ...
        # take FULL backup
        self.backup_node(backup_dir, "node", node, options=["--stream", "--note=hello", "--ttl=20d"])
        ...
        self.assertNotIn("note", self.show_pb(backup_dir, "node", page_id))

        self.assertNotIn("expire-time", self.show_pb(backup_dir, "node", page_id))
    ...
    def test_multi_timeline_merge(self):
        ...
        try:
            ...
            node.safe_psql("postgres", "create extension amcheck_next")
        ...
        self.checkdb_node(backup_dir, "node", options=["--amcheck", "-d", "postgres", "-p", str(node.port)])

        self.checkdb_node(backup_dir, "node", options=["--amcheck", "-d", "postgres", "-p", str(node_restored.port)])
    ...
    def test_merge_page_header_map_retry(self):
        ...
        delta_id = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--stream"])
    ...
    def test_missing_data_file(self):
        ...
        gdb = self.merge_backup(backup_dir, "node", delta_id, options=["--log-level-file=VERBOSE"], gdb=True)
        gdb.set_breakpoint("merge_files")
        gdb.run_until_break()

        # remove data file in incremental backup
        file_to_remove = os.path.join(backup_dir, "backups", "node", delta_id, "database", path)
        ...
        self.assertIn(
            'ERROR: Cannot open backup file "{0}": No such file or directory'.format(file_to_remove), logfile_content
        )
    ...
    def test_missing_non_data_file(self):
        ...
        # remove data file in incremental backup
        file_to_remove = os.path.join(backup_dir, "backups", "node", delta_id, "database", "backup_label")
        ...
        self.assertIn('ERROR: File "{0}" is not found'.format(file_to_remove), logfile_content)

        self.assertIn("ERROR: Backup files merging failed", logfile_content)
    ...
    def test_merge_remote_mode(self):
        ...
        with open(os.path.join(backups, full_id, "backup.control"), "a") as conf:
            conf.write("recovery_time='{:%Y-%m-%d %H:%M:%S}'\n".format(datetime.now() - timedelta(days=5)))

        gdb = self.backup_node(
            backup_dir, "node", node, options=["--log-level-file=VERBOSE", "--merge-expired"], gdb=True
        )
        gdb.set_breakpoint("merge_files")
        gdb.run_until_break()
        ...
        self.assertNotIn("SSH", logfile_content)
    ...
    def test_unfinished_merge(self):
        ...
        self._check_gdb_flag_or_skip_test()
        cases = [
            ("fail_merged", "write_backup_filelist", ["MERGED", "MERGING", "OK"]),
        ...
        for name, terminate_at, states in cases:
            ...
            with self.assertRaisesRegex(
                ProbackupException, f"Full backup {full_id} has unfinished merge with backup {backup_id}"
            ):
