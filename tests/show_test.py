...
class ShowTest(ProbackupTest, unittest.TestCase):
    ...
    def test_show_1(self):
        ...
        self.assertEqual(self.backup_node(backup_dir, "node", node, options=["--log-level-console=off"]), None)
    ...
    def test_no_control_file(self):
        ...
        file = os.path.join(backup_dir, "backups", "node", backup_id, "backup.control")
        os.remove(file)

        output = self.show_pb(backup_dir, "node", as_text=True, as_json=False)

        self.assertIn("Control file", output)
    ...
    def test_corrupt_control_file(self):
        ...
        self.assertIn(
            'WARNING: Invalid option "statuss" in file', self.show_pb(backup_dir, "node", as_json=False, as_text=True)
        )
    ...
    def test_corrupt_correctness(self):
        ...
        # FULL
        backup_local_id = self.backup_node(backup_dir, "node", node, no_remote=True)

        output_local = self.show_pb(backup_dir, "node", as_json=False, backup_id=backup_local_id)

        backup_remote_id = self.backup_node(backup_dir, "node", node)

        output_remote = self.show_pb(backup_dir, "node", as_json=False, backup_id=backup_remote_id)

        # check correctness
        self.assertEqual(output_local["data-bytes"], output_remote["data-bytes"])

        self.assertEqual(output_local["uncompressed-bytes"], output_remote["uncompressed-bytes"])

        # DELTA
        backup_local_id = self.backup_node(backup_dir, "node", node, backup_type="delta", no_remote=True)

        output_local = self.show_pb(backup_dir, "node", as_json=False, backup_id=backup_local_id)
        self.delete_pb(backup_dir, "node", backup_local_id)

        backup_remote_id = self.backup_node(backup_dir, "node", node, backup_type="delta")
        ...
        # PAGE
        backup_local_id = self.backup_node(backup_dir, "node", node, backup_type="page", no_remote=True)

        output_local = self.show_pb(backup_dir, "node", as_json=False, backup_id=backup_local_id)
        self.delete_pb(backup_dir, "node", backup_local_id)

        backup_remote_id = self.backup_node(backup_dir, "node", node, backup_type="page")
    ...
    def test_corrupt_correctness_2(self):
        ...
        # FULL
        backup_local_id = self.backup_node(backup_dir, "node", node, options=["--compress"], no_remote=True)

        output_local = self.show_pb(backup_dir, "node", as_json=False, backup_id=backup_local_id)

        if self.remote:
            backup_remote_id = self.backup_node(backup_dir, "node", node, options=["--compress"])
        else:
            backup_remote_id = self.backup_node(
                backup_dir, "node", node, options=["--remote-proto=ssh", "--remote-host=localhost", "--compress"]
            )
        ...
        backup_local_id = self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--compress"], no_remote=True
        )
        ...
        if self.remote:
            backup_remote_id = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--compress"])
        else:
            backup_remote_id = self.backup_node(
                backup_dir,
                "node",
        ...
        # PAGE
        backup_local_id = self.backup_node(
            backup_dir, "node", node, backup_type="page", options=["--compress"], no_remote=True
        )
        ...
        if self.remote:
            backup_remote_id = self.backup_node(backup_dir, "node", node, backup_type="page", options=["--compress"])
        else:
            backup_remote_id = self.backup_node(
                backup_dir,
                "node",
    ...
    def test_color_with_no_terminal(self):
        ...
        # FULL
        try:
            self.backup_node(backup_dir, "node", node, options=["--archive-timeout=1s"])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_tablespace_print_issue_431(self):
        ...
        self.assertIn(
            "tablespace_map", self.show_pb(backup_dir, "node", backup_id=full_backup_id, as_text=True, as_json=False)
        )
        self.assertIn(
            tblspc_path, self.show_pb(backup_dir, "node", backup_id=full_backup_id, as_text=True, as_json=False)
        )
