...
class ConfigTest(ProbackupTest, unittest.TestCase):
    ...
    def test_remove_instance_config(self):
        ...
        try:
            ...
            self.assertIn(
                'ERROR: could not open file "{0}": No such file or directory'.format(conf_file),
                e.message,
    ...
    def test_corrupt_backup_content(self):
        ...
        fulle1_conf_file = os.path.join(backup_dir, "backups", "node", full1_id, "backup_content.control")

        fulle2_conf_file = os.path.join(backup_dir, "backups", "node", fulle2_id, "backup_content.control")
