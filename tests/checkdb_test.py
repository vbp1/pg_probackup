...
class CheckdbTest(ProbackupTest, unittest.TestCase):
    ...
    def test_checkdb_amcheck_only_sanity(self):
        ...
        node.safe_psql("postgres", "create index on t_heap(id)")

        node.safe_psql("postgres", "create table idxpart (a int) partition by range (a)")

        # there aren't partitioned indexes on 10 and lesser versions
        if self.get_version(node) >= 110000:
            node.safe_psql("postgres", "create index on idxpart(a)")
        ...
        # simple sanity
        try:
            self.checkdb_node(options=["--skip-block-validation"])
            ...
            self.assertIn(
                "ERROR: Option '--skip-block-validation' must be used with '--amcheck' option",
                e.message,
        ...
        output = self.checkdb_node(
            options=["--amcheck", "--skip-block-validation", "-d", "postgres", "-p", str(node.port)]
        )

        self.assertIn("INFO: checkdb --amcheck finished successfully", output)
        self.assertIn("All checked indexes are valid", output)
        ...
        with open(log_file_path) as f:
            ...
            self.assertIn("INFO: checkdb --amcheck finished successfully", log_file_content)
            self.assertIn("VERBOSE: (query)", log_file_content)
        ...
        self.checkdb_node(
            ...
            options=[
                ...
                "--skip-block-validation",
                "--log-level-file=verbose",
                "--log-directory={0}".format(os.path.join(backup_dir, "log")),
                "-d",
                "postgres",
                "-p",
                str(node.port),
        ...
        with open(log_file_path) as f:
            log_file_content = f.read()
            self.assertIn("ERROR: checkdb --amcheck finished with failure", log_file_content)
            self.assertIn(
                "WARNING: Thread [1]. Amcheck failed in database 'postgres' for index: 'public.t_heap_id_idx':",
                log_file_content,
            )
            self.assertIn("ERROR:  could not open relation with OID", log_file_content)
    ...
    def test_basic_checkdb_amcheck_only_sanity(self):
        ...
        try:
            ...
            node.safe_psql("db1", "create extension amcheck_next")

        node.safe_psql("postgres", "create database db2")
        try:
            node.safe_psql("db2", "create extension amcheck")
        except QueryException:
            node.safe_psql("db2", "create extension amcheck_next")
        ...
        node.safe_psql("db2", "alter index pgbench_accounts_pkey rename to some_index")

        index_path_1 = os.path.join(
            node.data_dir,
            node.safe_psql("db1", "select pg_relation_filepath('pgbench_accounts_pkey')").decode("utf-8").rstrip(),
        )

        index_path_2 = os.path.join(
            node.data_dir, node.safe_psql("db2", "select pg_relation_filepath('some_index')").decode("utf-8").rstrip()
        )

        try:
            self.checkdb_node(options=["--amcheck", "--skip-block-validation", "-d", "postgres", "-p", str(node.port)])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
                "Expecting Error because some db was not amchecked Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
        ...
        with open(log_file_path) as f:
            log_file_content = f.read()
            self.assertIn(
                "WARNING: Thread [1]. Amcheck failed in database 'db1' for index: 'public.pgbench_accounts_pkey':",
                log_file_content,
            )

            self.assertIn(
                "WARNING: Thread [1]. Amcheck failed in database 'db2' for index: 'public.some_index':",
                log_file_content,
            )
    ...
    def test_checkdb_block_validation_sanity(self):
        ...
        self.checkdb_node(data_dir=node.data_dir, options=["-d", "postgres", "-p", str(node.port)])

        self.checkdb_node(backup_dir, "node", options=["-d", "postgres", "-p", str(node.port)])
        ...
        try:
            ...
            self.assertIn(
                'WARNING: Corruption detected in file "{0}", block 1'.format(os.path.normpath(heap_full_path)),
                e.message,
            )

            self.assertIn(
                'WARNING: Corruption detected in file "{0}", block 5'.format(os.path.normpath(heap_full_path)),
                e.message,
            )
    ...
    def test_checkdb_checkunique(self):
        ...
        os.path.join(self.tmp_path, self.module_name, self.fname, "backup")
        node = self.make_simple_node(
        ...
        try:
            ...
            if ProbackupTest.enterprise and (
                self.get_version(node) >= 111300
                and self.get_version(node) < 120000
                or self.get_version(node) >= 120800
                and self.get_version(node) < 130000
    ...
    def test_checkdb_with_least_privileges(self):
        ...
        try:
            node.safe_psql("backupdb", "create extension amcheck")
        except QueryException:
            node.safe_psql("backupdb", "create extension amcheck_next")
        ...
        # PG 10
            ...
            if ProbackupTest.enterprise:
                # amcheck-1.1
                node.safe_psql("backupdb", "GRANT EXECUTE ON FUNCTION bt_index_check(regclass, bool) TO backup")
            else:
                # amcheck-1.0
                node.safe_psql("backupdb", "GRANT EXECUTE ON FUNCTION bt_index_check(regclass) TO backup")
        # >= 11 < 14
            ...
            if ProbackupTest.enterprise:
                if (
                    ...
                    node.safe_psql(
                        "backupdb", "GRANT EXECUTE ON FUNCTION bt_index_check(regclass, bool, bool) TO backup"
                    )
        # >= 14
        else:
            node.safe_psql(
            ...
            # checkunique parameter
            if ProbackupTest.enterprise:
                node.safe_psql("backupdb", "GRANT EXECUTE ON FUNCTION bt_index_check(regclass, bool, bool) TO backup")
        ...
        try:
            self.checkdb_node(
                backup_dir, "node", options=["--amcheck", "-U", "backup", "-d", "backupdb", "-p", str(node.port)]
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
