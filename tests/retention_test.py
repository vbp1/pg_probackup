...
class RetentionTest(ProbackupTest, unittest.TestCase):
    ...
    def test_retention_redundancy_1(self):
        ...
        self.set_config(backup_dir, "node", options=["--retention-redundancy=1"])
        ...
        # Purge backups
        self.delete_expired(backup_dir, "node", options=["--expired", "--wal"])
        self.assertEqual(len(self.show_pb(backup_dir, "node")), 2)

        output_after = self.show_archive(backup_dir, "node", tli=1)

        self.assertEqual(output_before["max-segno"], output_after["max-segno"])

        self.assertNotEqual(output_before["min-segno"], output_after["min-segno"])
    ...
    def test_retention_window_2(self):
        ...
        for backup in os.listdir(backups):
            ...
            with open(os.path.join(backups, backup, "backup.control"), "a") as conf:
                conf.write("recovery_time='{:%Y-%m-%d %H:%M:%S}'\n".format(datetime.now() - timedelta(days=days_delta)))
                days_delta -= 1
    ...
    def test_retention_window_3(self):
        ...
        # Purge backups
        self.delete_expired(backup_dir, "node", options=["--retention-window=1", "--expired"])
    ...
    def test_retention_window_4(self):
        ...
        # Purge backups
        self.delete_expired(backup_dir, "node", options=["--retention-window=1", "--expired", "--wal"])
    ...
    def test_window_expire_interleaved_incremental_chains(self):
        ...
        # Take PAGEa1 backup
        page_id_a1 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        page_id_b1 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        page_id_a2 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        page_id_b2 = self.backup_node(backup_dir, "node", node, backup_type="page")
    ...
    def test_redundancy_expire_interleaved_incremental_chains(self):
        ...
        self.delete_expired(backup_dir, "node", options=["--retention-redundancy=1", "--expired"])
    ...
    def test_window_merge_interleaved_incremental_chains(self):
        ...
        output = self.delete_expired(
            backup_dir, "node", options=["--retention-window=1", "--expired", "--merge-expired"]
        )

        self.assertIn(
            "Merge incremental chain between full backup {0} and backup {1}".format(backup_id_a, page_id_a2), output
        )

        self.assertIn("Rename merged full backup {0} to {1}".format(backup_id_a, page_id_a2), output)

        self.assertIn(
            "Merge incremental chain between full backup {0} and backup {1}".format(backup_id_b, page_id_b2), output
        )

        self.assertIn("Rename merged full backup {0} to {1}".format(backup_id_b, page_id_b2), output)
    ...
    def test_window_merge_interleaved_incremental_chains_1(self):
        ...
        page_id_b3 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        self.delete_expired(backup_dir, "node", options=["--retention-window=1", "--expired", "--merge-expired"])

        self.assertEqual(len(self.show_pb(backup_dir, "node")), 2)

        self.assertEqual(self.show_pb(backup_dir, "node")[1]["id"], page_id_b3)

        self.assertEqual(self.show_pb(backup_dir, "node")[0]["id"], page_id_a1)

        self.assertEqual(self.show_pb(backup_dir, "node")[1]["backup-mode"], "FULL")

        self.assertEqual(self.show_pb(backup_dir, "node")[0]["backup-mode"], "FULL")
    ...
    def test_basic_window_merge_multiple_descendants(self):
        ...
        page_id_a3 = self.backup_node(backup_dir, "node", node, backup_type="page")
        # pgbench = node.pgbench(options=['-T', '10', '-c', '2'])
        # pgbench.wait()
        ...
        self.assertEqual(self.show_pb(backup_dir, "node", backup_id=page_id_a3)["parent-backup-id"], page_id_a1)

        self.assertEqual(self.show_pb(backup_dir, "node", backup_id=page_id_a2)["parent-backup-id"], page_id_a1)
        ...
        output = self.delete_expired(
            backup_dir,
            "node",
            options=["--retention-window=1", "--delete-expired", "--merge-expired", "--log-level-console=log"],
        )
        ...
        # Merging chain A
        self.assertIn(
            "Merge incremental chain between full backup {0} and backup {1}".format(backup_id_a, page_id_a3), output
        )

        self.assertIn("INFO: Rename merged full backup {0} to {1}".format(backup_id_a, page_id_a3), output)
        ...
        self.assertIn("LOG: Consider backup {0} for purge".format(page_id_a2), output)

        # Merge chain B
        self.assertIn(
            "Merge incremental chain between full backup {0} and backup {1}".format(backup_id_b, page_id_b3), output
        )

        self.assertIn("INFO: Rename merged full backup {0} to {1}".format(backup_id_b, page_id_b3), output)

        self.assertIn("Delete: {0}".format(page_id_a2), output)

        self.assertEqual(self.show_pb(backup_dir, "node")[1]["id"], page_id_b3)

        self.assertEqual(self.show_pb(backup_dir, "node")[0]["id"], page_id_a3)
    ...
    def test_basic_window_merge_multiple_descendants_1(self):
        ...
        output = self.delete_expired(
            backup_dir, "node", options=["--retention-window=1", "--merge-expired", "--log-level-console=log"]
        )
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[2]["id"], page_id_b3)

        self.assertEqual(self.show_pb(backup_dir, "node")[1]["id"], page_id_a3)

        self.assertEqual(self.show_pb(backup_dir, "node")[0]["id"], page_id_a2)

        self.assertEqual(self.show_pb(backup_dir, "node")[2]["backup-mode"], "FULL")

        self.assertEqual(self.show_pb(backup_dir, "node")[1]["backup-mode"], "FULL")

        self.assertEqual(self.show_pb(backup_dir, "node")[0]["backup-mode"], "PAGE")

        output = self.delete_expired(
            backup_dir, "node", options=["--retention-window=1", "--delete-expired", "--log-level-console=log"]
        )
    ...
    def test_window_chains(self):
        ...
        page_id_b3 = self.backup_node(backup_dir, "node", node, backup_type="delta")
        ...
        self.delete_expired(
            backup_dir,
            "node",
            options=["--retention-window=1", "--expired", "--merge-expired", "--log-level-console=log"],
        )
    ...
    def test_window_chains_1(self):
        ...
        self.assertIn("There are no backups to delete by retention policy", output)

        self.assertIn("Retention merging finished", output)

        output = self.delete_expired(
            backup_dir, "node", options=["--retention-window=1", "--expired", "--log-level-console=log"]
        )

        self.assertEqual(len(self.show_pb(backup_dir, "node")), 1)

        self.assertIn("There are no backups to merge by retention policy", output)

        self.assertIn("Purging finished", output)
    ...
    def test_window_error_backups_1(self):
        ...
        # Take PAGE BACKUP
        gdb = self.backup_node(backup_dir, "node", node, backup_type="page", gdb=True)
        ...
        # Take DELTA backup
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--retention-window=2", "--delete-expired"]
        )
    ...
    def test_window_error_backups_2(self):
        ...
        if self.get_version(node) < 90600:
            node.safe_psql("postgres", "SELECT pg_catalog.pg_stop_backup()")
    ...
    def test_wal_purge_victim(self):
        ...
        try:
            ...
            self.assertIn("INFO: Backup {0} WAL segments are valid".format(full_id), e.message)
            self.assertIn("WARNING: Backup {0} has missing parent 0".format(page_id), e.message)
    ...
    def test_failed_merge_redundancy_retention(self):
        ...
        self.set_config(backup_dir, "node", options=["--retention-redundancy=2"])

        self.set_config(backup_dir, "node", options=["--retention-window=2"])
        ...
        backups = os.path.join(backup_dir, "backups", "node")
        with open(os.path.join(backups, full_id, "backup.control"), "a") as conf:
            conf.write("recovery_time='{:%Y-%m-%d %H:%M:%S}'\n".format(datetime.now() - timedelta(days=3)))

        # run retention merge
        self.delete_expired(backup_dir, "node", options=["--delete-expired"])

        self.assertEqual(
            "MERGING", self.show_pb(backup_dir, "node", full_id)["status"], 'Backup STATUS should be "MERGING"'
        )

        self.assertEqual(
            "MERGING", self.show_pb(backup_dir, "node", delta_id)["status"], 'Backup STATUS should be "MERGING"'
        )
    ...
    def test_wal_depth_1(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"archive_timeout": "30s", "checkpoint_timeout": "30s"},
        )
        ...
        # PAGE
        node.pgbench_init(scale=1)
        B2 = self.backup_node(backup_dir, "node", node, backup_type="page")

        # generate_some more data
        node.pgbench_init(scale=1)

        target_xid = node.safe_psql("postgres", "select txid_current()").decode("utf-8").rstrip()
        ...
        output = self.restore_node(
            ...
            "node",
            node_restored,
            options=["--recovery-target-xid={0}".format(target_xid), "--recovery-target-action=promote"],
        )

        self.assertIn("Restore of backup {0} completed".format(B2), output)
        ...
        target_xid = node_restored.safe_psql("postgres", "select txid_current()").decode("utf-8").rstrip()
        ...
        node_restored.pgbench_init(scale=1)
        self.backup_node(backup_dir, "node", node_restored, data_dir=node_restored.data_dir)
        ...
        self.validate_pb(backup_dir, "node", backup_id=B2, options=["--recovery-target-lsn={0}".format(lsn)])
    ...
    def test_wal_purge(self):
        ...
        # STREAM FULL
        stream_id = self.backup_node(backup_dir, "node", node, options=["--stream"])
        ...
        # TLI 2
        node_tli2 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_tli2"))
        node_tli2.cleanup()
        ...
        self.assertIn("INFO: Restore of backup {0} completed".format(B1), output)

        self.set_auto_conf(node_tli2, options={"port": node_tli2.port})
        node_tli2.slow_start()
        node_tli2.pgbench_init(scale=4)

        target_xid = node_tli2.safe_psql("postgres", "select txid_current()").decode("utf-8").rstrip()
        node_tli2.pgbench_init(scale=1)

        self.backup_node(backup_dir, "node", node_tli2, data_dir=node_tli2.data_dir)
        ...
        # TLI3
        node_tli3 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_tli3"))
        node_tli3.cleanup()
        ...
        # TLI4
        node_tli4 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_tli4"))
        node_tli4.cleanup()
        ...
        self.restore_node(
            ...
            node_tli4,
            backup_id=stream_id,
            options=["--recovery-target=immediate", "--recovery-target-action=promote"],
        )
        ...
        self.backup_node(backup_dir, "node", node_tli4, data_dir=node_tli4.data_dir)
        node_tli4.pgbench_init(scale=5)
        node_tli4.cleanup()

        # TLI5
        node_tli5 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_tli5"))
        node_tli5.cleanup()
        ...
        output = self.delete_pb(
            backup_dir, "node", options=["--delete-wal", "--dry-run", "--log-level-console=verbose"]
        )
        ...
        self.assertIn("INFO: On timeline 5 all files can be removed", output)
        ...
        output = self.delete_pb(backup_dir, "node", options=["--delete-wal", "--log-level-console=verbose"])
        ...
        self.assertIn("INFO: On timeline 5 all files will be removed", output)
        ...
        self.assertEqual(show_tli4_before["min-segno"], "000000040000000000000002")

        self.assertEqual(show_tli4_after["min-segno"], "000000040000000000000006")
    ...
    def test_wal_depth_2(self):
        ...
        B4 = self.backup_node(backup_dir, "node", node_tli2, data_dir=node_tli2.data_dir)
        node_tli2.pgbench_init(scale=3)
        ...
        output = self.delete_pb(
            backup_dir, "node", options=["--delete-wal", "--dry-run", "--wal-depth=2", "--log-level-console=verbose"]
        )

        start_lsn_B2 = self.show_pb(backup_dir, "node", B2)["start-lsn"]
        self.assertIn("On timeline 1 WAL is protected from purge at {0}".format(start_lsn_B2), output)

        self.assertIn(
            "LOG: Archive backup {0} to stay consistent protect from "
            "purge WAL interval between 000000010000000000000004 "
            "and 000000010000000000000005 on timeline 1".format(B1),
            output,
        )

        start_lsn_B4 = self.show_pb(backup_dir, "node", B4)["start-lsn"]
        self.assertIn("On timeline 2 WAL is protected from purge at {0}".format(start_lsn_B4), output)

        self.assertIn(
            "LOG: Timeline 3 to stay reachable from timeline 1 protect "
            "from purge WAL interval between 000000020000000000000006 and "
            "000000020000000000000009 on timeline 2",
            output,
        )
        ...
        self.assertIn(
            ...
            "000000010000000000000006 on timeline 1",
            output,
        )
        ...
        output = self.delete_pb(
            backup_dir, "node", options=["--delete-wal", "--wal-depth=2", "--log-level-console=verbose"]
        )
        ...
        self.assertEqual(show_tli1_after["lost-segments"][0]["begin-segno"], "000000010000000000000007")

        self.assertEqual(show_tli1_after["lost-segments"][0]["end-segno"], "00000001000000000000000A")

        self.assertEqual(show_tli2_after["lost-segments"][0]["begin-segno"], "00000002000000000000000A")

        self.assertEqual(show_tli2_after["lost-segments"][0]["end-segno"], "00000002000000000000000A")
    ...
    def test_basic_wal_depth(self):
        ...
        pgbench = node.pgbench(options=["-T", "10", "-c", "2"])
        pgbench.wait()
        B3 = self.backup_node(backup_dir, "node", node, backup_type="page")

        # B4
        pgbench = node.pgbench(options=["-T", "10", "-c", "2"])
        pgbench.wait()
        B4 = self.backup_node(backup_dir, "node", node, backup_type="page")

        # B5
        pgbench = node.pgbench(options=["-T", "10", "-c", "2"])
        pgbench.wait()
        B5 = self.backup_node(backup_dir, "node", node, backup_type="page", options=["--wal-depth=1", "--delete-wal"])
        ...
        output = self.validate_pb(backup_dir, "node", B5, options=["--recovery-target-xid={0}".format(target_xid)])

        print(output)

        self.assertIn("INFO: Backup validation completed successfully on time", output)

        self.assertIn("xid {0} and LSN".format(target_xid), output)

        for backup_id in [B1, B2, B3, B4]:
            try:
                self.validate_pb(
                    backup_dir, "node", backup_id, options=["--recovery-target-xid={0}".format(target_xid)]
                )
                # we should die here because exception is what we expect to happen
                self.assertEqual(
                    1,
                ...
                self.assertIn("ERROR: Not enough WAL records to xid {0}".format(target_xid), e.message)
    ...
    def test_concurrent_running_full_backup(self):
        ...
        self.assertTrue(self.show_pb(backup_dir, "node")[0]["status"], "RUNNING")

        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--retention-redundancy=2", "--delete-expired"]
        )

        self.assertTrue(self.show_pb(backup_dir, "node")[1]["status"], "RUNNING")
