...
class BackupTest(ProbackupTest, unittest.TestCase):
    def test_full_backup(self):
        ...
        self.assertIn("INFO: Backup {0} is valid".format(backup_id), out)
    ...
    def test_backup_modes_archive(self):
        ...
        excluded = True
        db_dir = os.path.join(backup_dir, "backups", "node", full_backup_id, "database")

        for f in os.listdir(db_dir):
            if os.path.isfile(os.path.join(db_dir, f)) and (f == "postmaster.pid" or f == "postmaster.opts"):
                excluded = False
        ...
        # Check parent backup
        self.assertEqual(
            full_backup_id, self.show_pb(backup_dir, "node", backup_id=show_backup_1["id"])["parent-backup-id"]
        )

        self.assertEqual(
            page_backup_id, self.show_pb(backup_dir, "node", backup_id=show_backup_2["id"])["parent-backup-id"]
        )
    ...
    def test_incremental_backup_without_full(self):
        ...
        try:
            ...
            self.assertTrue(
                "WARNING: Valid full backup on current timeline 1 is not found" in e.message
                and "ERROR: Create new full backup before an incremental one" in e.message,
    ...
    def test_incremental_backup_corrupt_full(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "WARNING: Backup file" in e.message
                and "is not found" in e.message
                and "WARNING: Backup {0} data files are corrupted".format(backup_id) in e.message
    ...
    def test_delta_threads_stream(self):
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[0]["status"], "OK")
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream"])
    ...
    def test_page_detect_corruption(self):
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[1]["status"], "ERROR", "Backup Status should be ERROR")
    ...
    def test_backup_detect_corruption(self):
        ...
        node.safe_psql("postgres", "select count(*) from t_heap")

        node.safe_psql("postgres", "update t_heap set id = id + 10000")
        ...
        try:
            ...
            self.assertIn(
                ...
                "page verification failed, calculated checksum".format(heap_fullpath),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        try:
            self.backup_node(backup_dir, "node", node, backup_type="page", options=["-j", "4", "--stream"])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_backup_detect_invalid_block_header(self):
        ...
        try:
            ...
            self.assertIn(
                'ERROR: Corruption detected in file "{0}", block 1: page header invalid, pd_lower'.format(
                    heap_fullpath
                ),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
    ...
    def test_backup_truncate_misaligned(self):
        ...
        output = self.backup_node(
            backup_dir, "node", node, backup_type="full", options=["-j", "4", "--stream"], return_id=False
        )
    ...
    def test_tablespace_in_pgdata_pgpro_1376(self):
        ...
        self.create_tblspace_in_node(
            node, "tblspace1", tblspc_path=(os.path.join(node.data_dir, "somedirectory", "100500"))
        )

        self.create_tblspace_in_node(node, "tblspace2", tblspc_path=(os.path.join(node.data_dir)))
        ...
        backup_id_1 = self.backup_node(backup_dir, "node", node, backup_type="full", options=["-j", "4", "--stream"])

        node.safe_psql("postgres", "drop table t_heap2")
        node.safe_psql("postgres", "drop tablespace tblspace2")

        self.backup_node(backup_dir, "node", node, backup_type="full", options=["-j", "4", "--stream"])

        pgdata = self.pgdata_content(node.data_dir)

        relfilenode = node.safe_psql("postgres", "select 't_heap1'::regclass::oid").decode("utf-8").rstrip()

        list = []
        for root, _dirs, files in os.walk(os.path.join(backup_dir, "backups", "node", backup_id_1)):
            for file in files:
                if file == relfilenode:
    ...
    def test_basic_tablespace_handling(self):
        ...
        backup_id = self.backup_node(backup_dir, "node", node, backup_type="full", options=["-j", "4", "--stream"])

        tblspace1_old_path = self.get_tblspace_path(node, "tblspace1_old")
        tblspace2_old_path = self.get_tblspace_path(node, "tblspace2_old")

        self.create_tblspace_in_node(node, "some_lame_tablespace")

        self.create_tblspace_in_node(node, "tblspace1", tblspc_path=tblspace1_old_path)

        self.create_tblspace_in_node(node, "tblspace2", tblspc_path=tblspace2_old_path)
        ...
        try:
            self.restore_node(
                ...
                options=[
                    ...
                    "4",
                    "-T",
                    "{0}={1}".format(tblspace1_old_path, tblspace1_new_path),
                    "-T",
                    "{0}={1}".format(tblspace2_old_path, tblspace2_new_path),
                ],
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
            ...
            self.assertIn(
                "ERROR: Backup {0} has no tablespaceses, nothing to remap".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        node.safe_psql("postgres", "drop table t_heap_lame")

        node.safe_psql("postgres", "drop tablespace some_lame_tablespace")
    ...
    def test_tablespace_handling_1(self):
        ...
        try:
            self.restore_node(
                ...
                "node",
                node_restored,
                options=["-j", "4", "-T", "{0}={1}".format(tblspace2_old_path, tblspace_new_path)],
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
            ...
            self.assertTrue(
                "ERROR: --tablespace-mapping option" in e.message
                and "have an entry in tablespace_map file" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_tablespace_handling_2(self):
        ...
        try:
            self.restore_node(
                ...
                "node",
                node_restored,
                options=["-j", "4", "-T", "{0}={1}".format(tblspace1_old_path, tblspace_new_path)],
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
    ...
    def test_drop_rel_during_full_backup(self):
        ...
        for i in range(1, 512):
            node.safe_psql(
                "postgres", "create table t_heap_{0} as select i as id from generate_series(0,100) i".format(i)
            )
        ...
        relative_path_1 = node.safe_psql("postgres", "select pg_relation_filepath('t_heap_1')").decode("utf-8").rstrip()

        relative_path_2 = node.safe_psql("postgres", "select pg_relation_filepath('t_heap_1')").decode("utf-8").rstrip()

        os.path.join(node.data_dir, relative_path_1)
        os.path.join(node.data_dir, relative_path_2)
        ...
        # REMOVE file
        for i in range(1, 512):
            node.safe_psql("postgres", "drop table t_heap_{0}".format(i))
    ...
    def test_drop_db_during_full_backup(self):
        ...
        for i in range(1, 2):
            node.safe_psql("postgres", "create database t_heap_{0}".format(i))
        ...
        # REMOVE file
        for i in range(1, 2):
            node.safe_psql("postgres", "drop database t_heap_{0}".format(i))
    ...
    def test_drop_rel_during_backup_delta(self):
        ...
        node.safe_psql("postgres", "create table t_heap as select i as id from generate_series(0,100) i")

        relative_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
        ...
        # DELTA backup
        gdb = self.backup_node(
            backup_dir, "node", node, backup_type="delta", gdb=True, options=["--log-level-file=LOG"]
        )
        ...
        # REMOVE file
        node.safe_psql("postgres", "DROP TABLE t_heap")
    ...
    def test_drop_rel_during_backup_page(self):
        ...
        node.safe_psql("postgres", "insert into t_heap select i as id from generate_series(101,102) i")

        # PAGE backup
        gdb = self.backup_node(backup_dir, "node", node, backup_type="page", gdb=True, options=["--log-level-file=LOG"])
    ...
    def test_persistent_slot_for_stream_backup(self):
        ...
        node.safe_psql("postgres", "SELECT pg_create_physical_replication_slot('slot_1')")
    ...
    def test_basic_temp_slot_for_stream_backup(self):
        ...
        # FULL backup
        self.backup_node(backup_dir, "node", node, options=["--stream", "--temp-slot"])

        # FULL backup
        self.backup_node(backup_dir, "node", node, options=["--stream", "--slot=slot_1", "--temp-slot"])
    ...
    def test_backup_concurrent_drop_table(self):
        ...
        # FULL backup
        gdb = self.backup_node(backup_dir, "node", node, options=["--stream", "--compress"], gdb=True)
    ...
    def test_pg_11_adjusted_wal_segment_size(self):
        ...
        node = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node"),
            set_replication=True,
            initdb_params=["--data-checksums", "--wal-segsize=64"],
        ...
        # delete
        self.delete_pb(backup_dir, "node", options=["--expired", "--delete-wal", "--retention-redundancy=1"])
    ...
    def test_sigint_handling(self):
        ...
        # FULL backup
        gdb = self.backup_node(backup_dir, "node", node, gdb=True, options=["--stream", "--log-level-file=LOG"])
        ...
        self.assertEqual(
            "ERROR", self.show_pb(backup_dir, "node", backup_id)["status"], 'Backup STATUS should be "ERROR"'
        )
    ...
    def test_drop_table(self):
        ...
        connect_1.execute("create table t_heap as select i as id from generate_series(0,100) i")
        connect_1.commit()
    ...
    def test_basic_missing_file_permissions(self):
        ...
        relative_path = node.safe_psql("postgres", "select pg_relation_filepath('pg_class')").decode("utf-8").rstrip()
    ...
    def test_backup_with_least_privileges_role(self):
        ...
        if self.ptrack:
            node.safe_psql("backupdb", "CREATE SCHEMA ptrack; CREATE EXTENSION ptrack WITH SCHEMA ptrack")
        ...
        if self.ptrack:
            node.safe_psql("backupdb", "GRANT USAGE ON SCHEMA ptrack TO backup")
        ...
        # FULL backup
        self.backup_node(backup_dir, "node", node, datname="backupdb", options=["--stream", "-U", "backup"])
        self.backup_node(backup_dir, "node", node, datname="backupdb", options=["-U", "backup"])

        # PAGE
        self.backup_node(backup_dir, "node", node, backup_type="page", datname="backupdb", options=["-U", "backup"])
        self.backup_node(
            backup_dir, "node", node, backup_type="page", datname="backupdb", options=["--stream", "-U", "backup"]
        )

        # DELTA
        self.backup_node(backup_dir, "node", node, backup_type="delta", datname="backupdb", options=["-U", "backup"])
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", datname="backupdb", options=["--stream", "-U", "backup"]
        )

        # PTRACK
        if self.ptrack:
            self.backup_node(
                backup_dir, "node", node, backup_type="ptrack", datname="backupdb", options=["-U", "backup"]
            )
            self.backup_node(
                backup_dir, "node", node, backup_type="ptrack", datname="backupdb", options=["--stream", "-U", "backup"]
            )
    ...
    def test_parent_choosing(self):
        ...
        # PAGE1
        page1_id = self.backup_node(backup_dir, "node", node, backup_type="page")

        # PAGE2
        page2_id = self.backup_node(backup_dir, "node", node, backup_type="page")

        # Change PAGE1 to ERROR
        self.change_backup_status(backup_dir, "node", page1_id, "ERROR")

        # PAGE3
        page3_id = self.backup_node(backup_dir, "node", node, backup_type="page", options=["--log-level-file=LOG"])
        ...
        self.assertIn(
            "WARNING: Backup {0} has invalid parent: {1}. Cannot be a parent".format(page2_id, page1_id),
            log_file_content,
        )

        self.assertIn("WARNING: Backup {0} has status: ERROR. Cannot be a parent".format(page1_id), log_file_content)

        self.assertIn("Parent backup: {0}".format(full_id), log_file_content)

        self.assertEqual(self.show_pb(backup_dir, "node", backup_id=page3_id)["parent-backup-id"], full_id)
    ...
    def test_parent_choosing_1(self):
        ...
        # Delete PAGE1
        shutil.rmtree(os.path.join(backup_dir, "backups", "node", page1_id))
        ...
        self.assertIn(
            "WARNING: Backup {0} has missing parent: {1}. Cannot be a parent".format(page2_id, page1_id),
            log_file_content,
        )
    ...
    def test_parent_choosing_2(self):
        ...
        # Delete FULL
        shutil.rmtree(os.path.join(backup_dir, "backups", "node", full_id))

        # PAGE3
        try:
            self.backup_node(backup_dir, "node", node, backup_type="page", options=["--log-level-file=LOG"])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[2]["status"], "ERROR")
    ...
    def test_backup_with_less_privileges_role(self):
        ...
        if self.ptrack:
            node.safe_psql("backupdb", "CREATE EXTENSION ptrack")
        ...
        node.safe_psql("backupdb", "ALTER ROLE backup WITH REPLICATION;")
        ...
        self.set_config(backup_dir, "replica", options=["--archive-timeout=120s", "--log-level-console=LOG"])
        ...
        self.backup_node(backup_dir, "replica", replica, datname="backupdb", options=["-U", "backup"])

        # stream full backup from replica
        self.backup_node(backup_dir, "replica", replica, datname="backupdb", options=["--stream", "-U", "backup"])
        ...
        self.backup_node(
            ...
            backup_type="page",
            datname="backupdb",
            options=["-U", "backup", "--archive-timeout=30s"],
        )

        self.backup_node(
            backup_dir, "replica", replica, backup_type="page", datname="backupdb", options=["--stream", "-U", "backup"]
        )

        # DELTA backup from replica
        self.switch_wal_segment(node)
        self.backup_node(
            backup_dir, "replica", replica, backup_type="delta", datname="backupdb", options=["-U", "backup"]
        )
        self.backup_node(
            backup_dir,
            "replica",
        ...
        if self.ptrack:
            self.switch_wal_segment(node)
            self.backup_node(
                backup_dir, "replica", replica, backup_type="ptrack", datname="backupdb", options=["-U", "backup"]
            )
            self.backup_node(
                backup_dir,
                "replica",
    ...
    def test_issue_132(self):
        ...
        with node.connect("postgres") as conn:
            for i in range(50000):
                conn.execute("CREATE TABLE t_{0} as select 1".format(i))
                conn.commit()
    ...
    def test_issue_132_1(self):
        ...
        delta_id = self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--stream"], old_binary=True
        )
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", full_id)["status"], 'Backup STATUS should be "CORRUPT"'
        )

        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", delta_id)["status"], 'Backup STATUS should be "ORPHAN"'
        )

        # check that revalidation is working correctly
        try:
            self.restore_node(backup_dir, "node", node, backup_id=delta_id)
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
        ...
        output = self.restore_node(backup_dir, "node", node, backup_id=full_id, options=["--force"])

        self.assertIn("WARNING: Backup {0} has status: CORRUPT".format(full_id), output)

        self.assertIn("WARNING: Backup {0} is corrupt.".format(full_id), output)

        self.assertIn("WARNING: Backup {0} is not valid, restore is forced".format(full_id), output)

        self.assertIn("INFO: Restore of backup {0} completed.".format(full_id), output)

        node.cleanup()

        output = self.restore_node(backup_dir, "node", node, backup_id=delta_id, options=["--force"])

        self.assertIn("WARNING: Backup {0} is orphan.".format(delta_id), output)

        self.assertIn("WARNING: Backup {0} is not valid, restore is forced".format(full_id), output)

        self.assertIn("WARNING: Backup {0} is not valid, restore is forced".format(delta_id), output)

        self.assertIn("INFO: Restore of backup {0} completed.".format(delta_id), output)
    ...
    def test_note_sanity(self):
        ...
        backup_id = self.backup_node(
            backup_dir, "node", node, options=["--stream", "--log-level-file=LOG", "--note=test_note"]
        )
    ...
    def test_parent_backup_made_by_newer_version(self):
        ...
        try:
            ...
            self.assertIn(
                "pg_probackup do not guarantee to be forward compatible. Please upgrade pg_probackup binary.",
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_issue_290(self):
        ...
        try:
            self.backup_node(backup_dir, "node", node, options=["--archive-timeout=10s"])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_issue_203(self):
        ...
        self.restore_node(backup_dir, "node", node_restored, data_dir=node_restored.data_dir)
    ...
    def test_issue_231(self):
        ...
        os.path.join(node.data_dir, "123")
    ...
    def test_incr_backup_filenode_map(self):
        ...
        node1 = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node1"), initdb_params=["--data-checksums"]
        )
        node1.cleanup()
        ...
        node.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-T", "10", "-c", "1"])

        self.backup_node(backup_dir, "node", node, backup_type="delta")

        node.safe_psql("postgres", "reindex index pg_type_oid_index")
    ...
    def test_missing_wal_segment(self):
        ...
        gdb = self.backup_node(
            backup_dir, "node", node, datname="backupdb", options=["--stream", "--log-level-file=INFO"], gdb=True
        )
        ...
        self.assertIn("unexpected termination of replication stream: ERROR:  requested WAL segment", gdb.output)

        self.assertIn("has already been removed", gdb.output)

        self.assertIn("ERROR: Interrupted during waiting for WAL streaming", gdb.output)

        self.assertIn("WARNING: A backup is in progress, stopping it", gdb.output)
    ...
    def test_basic_backup_default_transaction_read_only(self):
        ...
        try:
            node.safe_psql("postgres", "create temp table t1()")
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_backup_atexit(self):
        ...
        # Full backup in streaming mode
        gdb = self.backup_node(backup_dir, "node", node, options=["--stream", "--log-level-file=VERBOSE"], gdb=True)
        ...
        with open(os.path.join(backup_dir, "log", "pg_probackup.log")) as f:
            log_content = f.read()
            # print(log_content)
            self.assertIn("WARNING: A backup is in progress, stopping it.", log_content)

            if self.get_version(node) < 150000:
                self.assertIn("FROM pg_catalog.pg_stop_backup", log_content)
            else:
                self.assertIn("FROM pg_catalog.pg_backup_stop", log_content)

            self.assertIn("setting its status to ERROR", log_content)
    ...
    def test_pg_stop_backup_missing_permissions(self):
        ...
        if self.get_version(node) < 90600:
            node.safe_psql("postgres", "REVOKE EXECUTE ON FUNCTION pg_catalog.pg_stop_backup() FROM backup")
        elif self.get_version(node) > 90600 and self.get_version(node) < 100000:
            node.safe_psql("postgres", "REVOKE EXECUTE ON FUNCTION pg_catalog.pg_stop_backup(boolean) FROM backup")
        elif self.get_version(node) < 150000:
            node.safe_psql(
                "postgres", "REVOKE EXECUTE ON FUNCTION pg_catalog.pg_stop_backup(boolean, boolean) FROM backup"
            )
        else:
            node.safe_psql("postgres", "REVOKE EXECUTE ON FUNCTION pg_catalog.pg_backup_stop(boolean) FROM backup")

        # Full backup in streaming mode
        try:
            self.backup_node(backup_dir, "node", node, options=["--stream", "-U", "backup"])
            # we should die here because exception is what we expect to happen
            if self.get_version(node) < 150000:
                self.assertEqual(
                    1,
    ...
    def test_start_time(self):
        ...
        try:
            ...
            self.backup_node(
                backup_dir, "node", node, backup_type="full", options=["--stream", "--start-time={0}".format(startTime)]
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
    ...
    def test_start_time_few_nodes(self):
        ...
        self.backup_node(
            backup_dir1, "node1", node1, backup_type="full", options=["--stream", "--start-time={0}".format(startTime)]
        )
        self.backup_node(
            backup_dir2, "node2", node2, backup_type="full", options=["--stream", "--start-time={0}".format(startTime)]
        )
        ...
        startTime = str(int(time()))
        self.backup_node(
            backup_dir1, "node1", node1, backup_type="delta", options=["--stream", "--start-time={0}".format(startTime)]
        )
        self.backup_node(
            backup_dir2, "node2", node2, backup_type="delta", options=["--stream", "--start-time={0}".format(startTime)]
        )
        ...
        startTime = str(int(time()))
        self.backup_node(
            backup_dir1, "node1", node1, backup_type="page", options=["--stream", "--start-time={0}".format(startTime)]
        )
        self.backup_node(
            backup_dir2, "node2", node2, backup_type="page", options=["--stream", "--start-time={0}".format(startTime)]
        )
        ...
        # PTRACK backup
        if self.ptrack:
            node1.safe_psql("postgres", "create extension ptrack")
            node2.safe_psql("postgres", "create extension ptrack")
    ...
    def test_regress_issue_585(self):
        ...
        with open(os.path.join(node.data_dir, "pg_multixact/offsets/1000"), "wb"):
            pass
        with open(os.path.join(node.data_dir, "pg_multixact/members/1000"), "wb"):
            pass
    ...
    def test_2_delta_backups(self):
        ...
        node = self.make_simple_node("node", initdb_params=["--data-checksums"])
        ...
        # delta backup mode
        delta_backup_id1 = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--stream"])

        delta_backup_id2 = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--stream"])
        ...
        self.assertFalse(os.path.exists(conf_file), "File should not exist: {0}".format(conf_file))
