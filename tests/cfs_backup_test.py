...
from .helpers.cfs_helpers import corrupt_file, find_by_extensions, find_by_name, find_by_pattern
...
class CfsBackupNoEncTest(ProbackupTest, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = self.make_simple_node(
            ...
            pg_options={"cfs_encryption": "off", "max_wal_senders": "2", "shared_buffers": "200MB"},
        )
        ...
        self.node.safe_psql("postgres", "CREATE EXTENSION ptrack")
        ...
        self.assertIn(
            tblspace_name, str(tblspace), "ERROR: The tablespace not created or it create without compressions"
        )

        self.assertIn(
            "compression=true", str(tblspace), "ERROR: The tablespace not created or it create without compressions"
        )
    ...
    def test_fullbackup_after_create_table(self):
        ...
        try:
            ...
            self.fail("\n ERROR: {0}\n CMD: {1}".format(repr(e.message), repr(self.cmd)))
            return False
        show_backup = self.show_pb(self.backup_dir, "node", backup_id)
        self.assertEqual(
            "OK",
        ...
        # check cfm size
        cfms = find_by_extensions([os.path.join(self.backup_dir, "backups", "node", backup_id)], [".cfm"])
        self.assertTrue(cfms, "ERROR: .cfm files not found in backup dir")
        for cfm in cfms:
            size = os.stat(cfm).st_size
            self.assertLessEqual(size, 4096, "ERROR: {0} is not truncated (has size {1} > 4096)".format(cfm, size))
    ...
    def test_fullbackup_empty_tablespace_ptrack_after_create_table(self):
        ...
        try:
            ...
            self.fail("ERROR: Incremental backup failed.\n {0} \n {1}".format(repr(self.cmd), repr(e.message)))
        show_backup = self.show_pb(self.backup_dir, "node", backup_id)
        self.assertEqual(
            "OK",
            show_backup["status"],
    ...
    def test_page_doesnt_store_unchanged_cfm(self):
        ...
        self.assertTrue(
            find_by_extensions([os.path.join(self.backup_dir, "backups", "node", backup_id_full)], [".cfm"]),
            "ERROR: .cfm files not found in backup dir",
        )
    ...
    def test_fullbackup_after_create_table_ptrack_after_create_table(self):
        ...
        self.assertGreater(
            ...
            "ERROR: Size of incremental backup greater than full. \n INFO: {0} >{1}".format(
                show_backup_ptrack["data-bytes"], show_backup_full["data-bytes"]
            ),
        )
    ...
    def test_fullbackup_after_create_table_ptrack_after_create_table_stream(self):
        ...
        try:
            backup_id_full = self.backup_node(
                self.backup_dir, "node", self.node, backup_type="full", options=["--stream"]
            )
        except ProbackupException as e:
            self.fail("ERROR: Full backup failed.\n {0} \n {1}".format(repr(self.cmd), repr(e.message)))
        ...
        try:
            backup_id_ptrack = self.backup_node(
                self.backup_dir, "node", self.node, backup_type="ptrack", options=["--stream"]
            )
        except ProbackupException as e:
            self.fail("ERROR: Incremental backup failed.\n {0} \n {1}".format(repr(self.cmd), repr(e.message)))
    ...
    def test_fullbackup_after_create_table_page_after_create_table(self):
        ...
        self.assertGreater(
            ...
            "ERROR: Size of incremental backup greater than full. \n INFO: {0} >{1}".format(
                show_backup_page["data-bytes"], show_backup_full["data-bytes"]
            ),
        )
    ...
    def test_multiple_segments(self):
        ...
        self.node.safe_psql(
            ...
            "AS SELECT i AS id, MD5(i::text) AS text, "
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,1005000) i".format("t_heap", tblspace_name),
        )
        ...
        self.node.safe_psql(
            ...
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,10) i".format("t_heap"),
        )
        ...
        self.restore_node(
            ...
            options=["-j", "4", "--recovery-target=immediate", "--recovery-target-action=promote"],
        )

        self.node.slow_start()
        self.assertEqual(full_result, self.node.table_checksum("t_heap"), "Lost data after restore")
        ...
        self.node.slow_start()
        self.assertEqual(page_result, self.node.table_checksum("t_heap"), "Lost data after restore")
    ...
    def test_multiple_segments_in_multiple_tablespaces(self):
        ...
        self.node.safe_psql(
            ...
            "AS SELECT i AS id, MD5(i::text) AS text, "
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,1005000) i".format("t_heap_1", tblspace_name_1),
        )
        ...
        self.node.safe_psql(
            ...
            "AS SELECT i AS id, MD5(i::text) AS text, "
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,1005000) i".format("t_heap_2", tblspace_name_2),
        )
        ...
        self.node.safe_psql(
            ...
            "SELECT i AS id, MD5(i::text) AS text, "
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,10) i".format("t_heap_1"),
        )
        ...
        self.node.safe_psql(
            ...
            "SELECT i AS id, MD5(i::text) AS text, "
            "MD5(repeat(i::text,10))::tsvector AS tsvector "
            "FROM generate_series(0,10) i".format("t_heap_2"),
        )
        ...
        self.assertEqual(full_result_1, self.node.table_checksum("t_heap_1"), "Lost data after restore")
        self.assertEqual(full_result_2, self.node.table_checksum("t_heap_2"), "Lost data after restore")
        ...
        self.assertEqual(page_result_1, self.node.table_checksum("t_heap_1"), "Lost data after restore")
        self.assertEqual(page_result_2, self.node.table_checksum("t_heap_2"), "Lost data after restore")
    ...
    def test_fullbackup_after_create_table_page_after_create_table_stream(self):
        ...
        try:
            backup_id_page = self.backup_node(
                self.backup_dir, "node", self.node, backup_type="page", options=["--stream"]
            )
        except ProbackupException as e:
            self.fail("ERROR: Incremental backup failed.\n {0} \n {1}".format(repr(self.cmd), repr(e.message)))
    ...
    def test_delete_random_cfm_file_from_tablespace_dir(self):
        ...
        self.assertTrue(list_cmf, "ERROR: .cfm-files not found into tablespace dir")

        os.remove(random.choice(list_cmf))

        self.assertRaises(ProbackupException, self.backup_node, self.backup_dir, "node", self.node, backup_type="full")
    ...
    def test_delete_random_data_file_from_tablespace_dir(self):
        ...
        list_data_files = find_by_pattern([self.get_tblspace_path(self.node, tblspace_name)], r"^.*/\d+$")
        self.assertTrue(list_data_files, "ERROR: Files of data not found into tablespace dir")
    ...
    def test_broken_file_pg_compression_into_tablespace_dir(self):
        ...
        self.assertTrue(corrupt_file(corrupted_file), "ERROR: File is not corrupted or it missing")
