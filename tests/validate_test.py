...
class ValidateTest(ProbackupTest, unittest.TestCase):
    ...
    def test_basic_validate_nullified_heap_page_backup(self):
        ...
        file_path = (
            node.safe_psql("postgres", "select pg_relation_filepath('pgbench_accounts')").decode("utf-8").rstrip()
        )
    ...
    def test_validate_wal_unreal_values(self):
        ...
        self.assertIn(
            ...
            self.validate_pb(backup_dir, "node", options=["--time={0}".format(target_time), "-j", "4"]),
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--time={0}".format(unreal_time_1), "-j", "4"])
            self.assertEqual(
                1,
                0,
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--time={0}".format(unreal_time_2), "-j", "4"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because of validation to unreal time.\n Output: {0} \n CMD: {1}".format(
        ...
        self.assertIn(
            "INFO: Backup validation completed successfully",
            self.validate_pb(backup_dir, "node", options=["--xid={0}".format(target_xid), "-j", "4"]),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--xid={0}".format(unreal_xid), "-j", "4"])
            self.assertEqual(
                1,
                0,
        ...
        output = self.validate_pb(backup_dir, "node", backup_id, options=["-j", "4"])
        self.assertIn(
    ...
    def test_basic_validate_corrupted_intermediate_backup(self):
        ...
        file_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
        # PAGE1
        ...
        # Simple validate
        try:
            self.validate_pb(backup_dir, "node", backup_id=backup_id_2, options=["-j", "4"])
            self.assertEqual(
                1,
                0,
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id_2)["status"], 'Backup STATUS should be "CORRUPT"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_3)["status"], 'Backup STATUS should be "ORPHAN"'
        )
    ...
    def test_validate_corrupted_intermediate_backups(self):
        ...
        file_path_t_heap = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
        # FULL
        ...
        file_path_t_heap_1 = (
            node.safe_psql("postgres", "select pg_relation_filepath('t_heap_1')").decode("utf-8").rstrip()
        )
        # PAGE1
        backup_id_2 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id_1)["status"], 'Backup STATUS should be "CORRUPT"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_2)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_3)["status"], 'Backup STATUS should be "ORPHAN"'
        )
    ...
    def test_validate_specific_error_intermediate_backups(self):
        ...
        control_path = os.path.join(backup_dir, "backups", "node", backup_id_1, "backup.control")
        ...
        for line in actual_control.splitlines():
            new_control_file += line.replace("status = OK", "status = ERROR")
            new_control_file += "\n"
        ...
        try:
            ...
            self.assertTrue(
                "WARNING: Backup {0} is orphaned because his parent {1} has status: ERROR".format(
                    backup_id_2, backup_id_1
                )
                in e.message
                and "INFO: Validating parents for backup {0}".format(backup_id_2) in e.message
                and "WARNING: Backup {0} has status ERROR. Skip validation.".format(backup_id_1)
                and "ERROR: Backup {0} is orphan.".format(backup_id_2) in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        self.assertEqual(
            "ERROR", self.show_pb(backup_dir, "node", backup_id_1)["status"], 'Backup STATUS should be "ERROR"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_2)["status"], 'Backup STATUS should be "ORPHAN"'
        )
    ...
    def test_validate_error_intermediate_backups(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                in e.message
                and "WARNING: Backup {0} has status ERROR. Skip validation".format(backup_id_1) in e.message
                and "WARNING: Some backups are not valid" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_validate_corrupted_intermediate_backups_1(self):
        ...
        file_page_2 = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
        backup_id_3 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        file_page_5 = node.safe_psql("postgres", "select pg_relation_filepath('t_heap1')").decode("utf-8").rstrip()
        ...
        # Validate PAGE3
        try:
            self.validate_pb(backup_dir, "node", backup_id=backup_id_4, options=["-j", "4"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because of data files corruption.\n Output: {0} \n CMD: {1}".format(
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id_3)["status"], 'Backup STATUS should be "CORRUPT"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_4)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_5)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_6)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_7)["status"], 'Backup STATUS should be "ORPHAN"'
        )
    ...
    def test_validate_specific_target_corrupted_intermediate_backups(self):
        ...
        target_xid = (
            node.safe_psql(
                ...
                "insert into t_heap select i as id, md5(i::text) as text, "
                "md5(repeat(i::text,10))::tsvector as tsvector "
                "from generate_series(30001, 30001) i  RETURNING (xmin)",
            )
            .decode("utf-8")
            .rstrip()
        )
        ...
        # Validate PAGE3
        try:
            self.validate_pb(backup_dir, "node", options=["-i", backup_id_4, "--xid={0}".format(target_xid), "-j", "4"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because of data files corruption.\n Output: {0} \n CMD: {1}".format(
    ...
    def test_validate_instance_with_several_corrupt_backups(self):
        ...
        node.safe_psql("postgres", "create table t_heap as select generate_series(0,1) i")
        # FULL1
        backup_id_1 = self.backup_node(backup_dir, "node", node, options=["--no-validate"])

        # FULL2
        backup_id_2 = self.backup_node(backup_dir, "node", node)
        rel_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()

        node.safe_psql("postgres", "insert into t_heap values(2)")
        ...
        node.safe_psql("postgres", "insert into t_heap values(3)")

        backup_id_5 = self.backup_node(backup_dir, "node", node, backup_type="page")

        # FULL4
        backup_id_6 = self.backup_node(backup_dir, "node", node, options=["--no-validate"])

        # Corrupt some files in FULL2 and FULL3 backup
        os.remove(os.path.join(backup_dir, "backups", "node", backup_id_2, "database", rel_path))
        os.remove(os.path.join(backup_dir, "backups", "node", backup_id_4, "database", rel_path))
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id_4)["status"], 'Backup STATUS should be "CORRUPT"'
        )
        self.assertEqual(
            "ORPHAN", self.show_pb(backup_dir, "node", backup_id_5)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual("OK", self.show_pb(backup_dir, "node", backup_id_6)["status"], 'Backup STATUS should be "OK"')
    ...
    def test_validate_instance_with_several_corrupt_backups_interrupt(self):
        ...
        # Corrupt some files in FULL2 and FULL3 backup
        os.remove(os.path.join(backup_dir, "backups", "node", backup_id_1, "database", rel_path))
        os.remove(os.path.join(backup_dir, "backups", "node", backup_id_3, "database", rel_path))

        # Validate Instance
        gdb = self.validate_pb(backup_dir, "node", options=["-j", "4", "--log-level-file=LOG"], gdb=True)
        ...
        self.assertEqual(
            "DONE", self.show_pb(backup_dir, "node", backup_id_1)["status"], 'Backup STATUS should be "OK"'
        )
        self.assertEqual("OK", self.show_pb(backup_dir, "node", backup_id_2)["status"], 'Backup STATUS should be "OK"')
        self.assertEqual(
            "OK", self.show_pb(backup_dir, "node", backup_id_3)["status"], 'Backup STATUS should be "CORRUPT"'
        )
        self.assertEqual(
            "OK", self.show_pb(backup_dir, "node", backup_id_4)["status"], 'Backup STATUS should be "ORPHAN"'
        )
        self.assertEqual("OK", self.show_pb(backup_dir, "node", backup_id_5)["status"], 'Backup STATUS should be "OK"')
        self.assertEqual(
            "DONE", self.show_pb(backup_dir, "node", backup_id_6)["status"], 'Backup STATUS should be "OK"'
        )

        log_file = os.path.join(backup_dir, "log", "pg_probackup.log")
        with open(log_file) as f:
            log_content = f.read()
            self.assertNotIn("Interrupted while locking backup", log_content)
    ...
    def test_validate_instance_with_corrupted_page(self):
        ...
        file_path_t_heap1 = (
            node.safe_psql("postgres", "select pg_relation_filepath('t_heap1')").decode("utf-8").rstrip()
        )
        # PAGE1
        backup_id_2 = self.backup_node(backup_dir, "node", node, backup_type="page")
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "WARNING: Backup {0} is orphaned because his parent {1} has status: CORRUPT".format(
                    backup_id_3, backup_id_2
                )
                in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_validate_instance_with_corrupted_full_and_try_restore(self):
        ...
        node.cleanup()
        restore_out = self.restore_node(backup_dir, "node", node, options=["--no-validate"])
        self.assertIn(
    ...
    def test_validate_corrupt_wal_1(self):
        ...
        wals = [
            f for f in os.listdir(wals_dir) if os.path.isfile(os.path.join(wals_dir, f)) and not f.endswith(".backup")
        ]
        wals.sort()
        for wal in wals:
        ...
        try:
            ...
            self.assertTrue(
                "WARNING: Backup" in e.message
                and "WAL segments are corrupted" in e.message
                and "WARNING: There are not enough WAL records to consistenly restore backup" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_validate_corrupt_wal_2(self):
        ...
        # Validate to xid
        try:
            self.validate_pb(backup_dir, "node", backup_id, options=["--xid={0}".format(target_xid), "-j", "4"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because of wal segments corruption.\n Output: {0} \n CMD: {1}".format(
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id)["status"], 'Backup STATUS should be "CORRUPT"'
        )
    ...
    def test_validate_wal_lost_segment_1(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "WARNING: There are not enough WAL records to consistenly restore backup {0}".format(backup_id)
                in e.message
                and "WARNING: Backup {0} WAL segments are corrupted".format(backup_id) in e.message
                and "WARNING: Some backups are not valid" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id)["status"], 'Backup {0} should have STATUS "CORRUPT"'
        )
        ...
        try:
            ...
            self.assertIn(
                "INFO: Revalidating backup {0}".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
                "ERROR: Backup {0} is corrupt.".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_validate_corrupt_wal_between_backups(self):
        ...
        if self.get_version(node) < self.version_to_num("10.0"):
            walfile = (
                node.safe_psql("postgres", "select pg_xlogfile_name(pg_current_xlog_location())")
                .decode("utf-8")
                .rstrip()
            )
            target_lsn = node.safe_psql("postgres", "select pg_current_xlog_location()").decode("utf-8").rstrip()
        else:
            walfile = (
                node.safe_psql("postgres", "select pg_walfile_name(pg_current_wal_lsn())").decode("utf-8").rstrip()
            )
            target_lsn = node.safe_psql("postgres", "select pg_current_wal_lsn()").decode("utf-8").rstrip()
        ...
        # Validate to xid
        try:
            self.validate_pb(backup_dir, "node", backup_id, options=[f"--xid={target_xid}", "-j", "4"])
            self.assertEqual(
                1,
                0,
                f"Expecting Error because of wal segments corruption.\n Output: {repr(self.output)} \n CMD: {self.cmd}",
            )
        except ProbackupException as e:
            self.assertTrue(
                "ERROR: Not enough WAL records to xid" in e.message
                and "WARNING: Recovery can be done up to time" in e.message
                and f"ERROR: Not enough WAL records to xid {target_xid}\n",
    ...
    def test_pgpro702_688(self):
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--time={0}".format(recovery_time), "-j", "4"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because of wal segment disappearance.\n Output: {0} \n CMD: {1}".format(
            ...
            self.assertIn(
                "WAL archive is empty. You cannot restore backup to a recovery target without WAL archive",
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_pgpro561(self):
        ...
        self.set_auto_conf(node2, {"port": node2.port, "archive_mode": "off"})

        node2.slow_start()

        self.set_auto_conf(node2, {"archive_mode": "on"})
        ...
        self.assertEqual(
            timeline_node1, timeline_node2, "Timelines on Master and Node1 should be equal. This is unexpected"
        )
        ...
        self.assertEqual(
            archive_command_node1,
            archive_command_node2,
            "Archive command on Master and Node should be equal. This is unexpected",
        )
        ...
        with open(log_file) as f:
            log_content = f.read()
            self.assertTrue(
                "LOG:  archive command failed with exit code 1" in log_content
                and "DETAIL:  The failed archive command was:" in log_content
                and "WAL file already exists in archive with different checksum" in log_content,
    ...
    def test_validate_corrupted_full(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "30"},
        )
        ...
        node.safe_psql("postgres", "alter system set archive_command = 'false'")
        node.reload()
        try:
            self.backup_node(backup_dir, "node", node, backup_type="page", options=["--archive-timeout=1s"])
            self.assertEqual(
                1,
                0,
        ...
        try:
            ...
            self.assertIn(
                "Validating backup {0}".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
                "WARNING: Backup {0} data files are corrupted".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
    ...
    def test_validate_corrupted_full_1(self):
        ...
        file = os.path.join(backup_dir, "backups", "node", backup_id_page, "database", "backup_label")
    ...
    def test_validate_corrupted_full_2(self):
        ...
        self.backup_node(backup_dir, "node", node)
        self.backup_node(backup_dir, "node", node, backup_type="page")
        corrupt_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")
        validate_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")

        file = os.path.join(backup_dir, "backups", "node", corrupt_id, "database", "backup_label")
        ...
        try:
            ...
            self.assertIn(
                "INFO: Validating backup {0}".format(self.show_pb(backup_dir, "node")[2]["id"]),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
                "INFO: Validating backup {0}".format(self.show_pb(backup_dir, "node")[3]["id"]),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
                "INFO: Validating backup {0}".format(corrupt_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            self.assertIn(
                "WARNING: Backup {0} data files are corrupted".format(corrupt_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        try:
            ...
            self.assertIn(
                "Backup {0} data files are valid".format(self.show_pb(backup_dir, "node")[9]["id"]),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

            self.assertIn(
                "Backup {0} data files are valid".format(self.show_pb(backup_dir, "node")[8]["id"]),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            ...
            self.assertIn(
                "INFO: Revalidating backup {0}".format(corrupt_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        try:
            ...
            self.assertIn(
                "ERROR: Backup {0} is orphan.".format(validate_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        output = self.validate_pb(backup_dir, "node", validate_id, options=["-j", "4"])
        ...
        self.assertIn(
            "Backup {0} data files are valid".format(corrupt_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "INFO: Revalidating backup {0}".format(self.show_pb(backup_dir, "node")[5]["id"]),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "Backup {0} data files are valid".format(self.show_pb(backup_dir, "node")[5]["id"]),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "INFO: Revalidating backup {0}".format(validate_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "Backup {0} data files are valid".format(validate_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "INFO: Backup {0} WAL segments are valid".format(validate_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "INFO: Backup {0} is valid.".format(validate_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "INFO: Validate of backup {0} completed.".format(validate_id),
            output,
            "\n Unexpected Output Message: {0}\n".format(repr(output)),
        )
    ...
    def test_validate_corrupted_full_missing(self):
        ...
        # break PAGE
        old_directory = os.path.join(backup_dir, "backups", "node", backup_id_page)
        ...
        try:
            ...
            self.assertIn(
                "WARNING: Backup {0} has missing parent {1}".format(
                    self.show_pb(backup_dir, "node")[7]["id"], backup_id_page
                ),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
            ...
            self.assertIn(
                "WARNING: Backup {0} has missing parent {1}".format(
                    self.show_pb(backup_dir, "node")[6]["id"], backup_id_page
                ),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        self.assertIn("INFO: All backups are valid", output, "\n Unexpected Error Message: {0}\n".format(repr(output)))

        self.assertIn(
            "WARNING: Backup {0} has parent {1} with status: ORPHAN".format(
                self.show_pb(backup_dir, "node")[8]["id"], self.show_pb(backup_dir, "node")[6]["id"]
            ),
            output,
            "\n Unexpected Error Message: {0}\n".format(repr(output)),
        )
        ...
        self.assertIn(
            "WARNING: Backup {0} has parent {1} with status: ORPHAN".format(
                self.show_pb(backup_dir, "node")[7]["id"], self.show_pb(backup_dir, "node")[6]["id"]
            ),
            output,
            "\n Unexpected Error Message: {0}\n".format(repr(output)),
        )
        ...
        self.assertIn(
            "Revalidating backup {0}".format(self.show_pb(backup_dir, "node")[6]["id"]),
            output,
            "\n Unexpected Error Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "Revalidating backup {0}".format(self.show_pb(backup_dir, "node")[7]["id"]),
            output,
            "\n Unexpected Error Message: {0}\n".format(repr(output)),
        )

        self.assertIn(
            "Revalidating backup {0}".format(self.show_pb(backup_dir, "node")[8]["id"]),
            output,
            "\n Unexpected Error Message: {0}\n".format(repr(output)),
        )
    ...
    def test_file_size_corruption_no_validate(self):
        ...
        heap_path = node.safe_psql("postgres", "select pg_relation_filepath('t_heap')").decode("utf-8").rstrip()
        heap_size = node.safe_psql("postgres", "select pg_relation_size('t_heap')")

        backup_id = self.backup_node(
            backup_dir, "node", node, backup_type="full", options=["-j", "4"], asynchronous=False, gdb=False
        )
        ...
        try:
            ...
            self.assertTrue("ERROR: Backup files restoring failed" in e.message, repr(e.message))
    ...
    def test_validate_specific_backup_with_missing_backup(self):
        ...
        self.backup_node(backup_dir, "node", node)
        self.backup_node(backup_dir, "node", node, backup_type="page")
        missing_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")
        validate_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")
    ...
    def test_validate_specific_backup_with_missing_backup_1(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="page")
        missing_page_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")
        validate_id = self.backup_node(backup_dir, "node", node, backup_type="page")
        self.backup_node(backup_dir, "node", node, backup_type="page")
    ...
    def test_corrupt_pg_control_via_resetxlog(self):
        ...
        os.mkdir(os.path.join(backup_dir, "backups", "node", backup_id, "database", wal_dir, "archive_status"))

        os.mkdir(os.path.join(backup_dir, "backups", "node", backup_id, "database", wal_dir, "summaries"))

        pg_control_path = os.path.join(backup_dir, "backups", "node", backup_id, "database", "global", "pg_control")

        md5_before = hashlib.md5(open(pg_control_path, "rb").read()).hexdigest()
        ...
        md5_after = hashlib.md5(open(pg_control_path, "rb").read()).hexdigest()

        if self.verbose:
            print("\n MD5 BEFORE resetxlog: {0}\n MD5 AFTER resetxlog: {1}".format(md5_before, md5_after))
    ...
    def test_validation_after_backup(self):
        ...
        # FULL backup
        gdb = self.backup_node(backup_dir, "node", node, gdb=True, options=["--stream"])
        ...
        self.assertEqual(
            "CORRUPT", self.show_pb(backup_dir, "node", backup_id)["status"], 'Backup STATUS should be "ERROR"'
        )
    ...
    def test_validate_corrupt_tablespace_map(self):
        ...
        node.safe_psql("postgres", 'CREATE TABLE t_heap(a int) TABLESPACE "external_dir"')

        # FULL backup
        backup_id = self.backup_node(backup_dir, "node", node, options=["--stream"])

        tablespace_map = os.path.join(backup_dir, "backups", "node", backup_id, "database", "tablespace_map")
    ...
    def test_validate_target_lsn(self):
        ...
        self.validate_pb(
            backup_dir, "node", options=["--recovery-target-timeline=2", "--recovery-target-lsn={0}".format(target_lsn)]
        )
    ...
    def test_partial_validate_empty_and_mangled_database_map(self):
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--db-include=db1"])
            self.assertEqual(
                1,
                0,
    ...
    def test_partial_validate_exclude(self):
        ...
        try:
            self.validate_pb(backup_dir, "node", options=["--db-include=db1", "--db-exclude=db2"])
            self.assertEqual(
                1,
                0,
        ...
        try:
            self.validate_pb(
                backup_dir, "node", options=["--db-exclude=db1", "--db-exclude=db5", "--log-level-console=verbose"]
            )
            self.assertEqual(
                1,
                0,
        ...
        output = self.validate_pb(
            ...
            "node",
            backup_id,
            options=["--db-exclude=db1", "--db-exclude=db5", "--log-level-console=verbose"],
        )

        self.assertIn("VERBOSE: Skip file validation due to partial restore", output)
    ...
    def test_partial_validate_include(self):
        ...
        output = self.validate_pb(
            ...
            "node",
            backup_id,
            options=["--db-include=db1", "--db-include=db5", "--db-include=postgres", "--log-level-console=verbose"],
        )

        self.assertIn("VERBOSE: Skip file validation due to partial restore", output)

        output = self.validate_pb(backup_dir, "node", backup_id, options=["--log-level-console=verbose"])

        self.assertNotIn("VERBOSE: Skip file validation due to partial restore", output)
    ...
    def test_not_validate_diffenent_pg_version(self):
        ...
        control_file = os.path.join(backup_dir, "backups", "node", backup_id, "backup.control")
        ...
        data = data.replace(
            "server-version = {0}".format(str(pg_version)), "server-version = {0}".format(str(fake_new_pg_version))
        )
    ...
    def test_validate_corrupt_page_header_map(self):
        ...
        page_header_map = os.path.join(backup_dir, "backups", "node", backup_id, "page_header_map")
    ...
    def test_no_validate_tablespace_map(self):
        ...
        oid = (
            node.safe_psql("postgres", "select oid from pg_tablespace where spcname = 'external_dir'")
            .decode("utf-8")
            .rstrip()
        )
        ...
        self.assertTrue(os.path.islink(tablespace_link), "'%s' is not a symlink" % tablespace_link)
