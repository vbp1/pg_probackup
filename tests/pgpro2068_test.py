...
class BugTest(ProbackupTest, unittest.TestCase):
    def test_minrecpoint_on_replica(self):
        ...
        self.set_auto_conf(replica, {"port": replica.port, "restart_after_crash": "off"})
        ...
        pgbench = node.pgbench(
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "4", "-j 4", "-T", "100"]
        )
        ...
        replica.append_conf(recovery_config, "recovery_target = 'immediate'")
        replica.append_conf(recovery_config, "recovery_target_action = 'pause'")
        replica.slow_start(replica=True)
...
""".format(current_xlog_lsn_query=current_xlog_lsn_query)
        ...
        replica.safe_psql("postgres", script)
