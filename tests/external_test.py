...
class ExternalTest(ProbackupTest, unittest.TestCase):
    ...
    def test_basic_external(self):
        ...
        try:
            self.backup_node(
                backup_dir, "node", node, backup_type="full", options=["--external-dirs={0}".format(file_path)]
            )
            ...
            self.assertTrue(
                "ERROR:  --external-dirs option" in e.message and "directory or symbolic link expected" in e.message,
        ...
        self.restore_node(backup_dir, "node", node, data_dir=external_dir, options=["-j", "4"])

        # Full backup with external dir
        self.backup_node(backup_dir, "node", node, options=["--external-dirs={0}".format(external_dir)])
    ...
    def test_external_none(self):
        ...
        # Full backup with external dir
        self.backup_node(backup_dir, "node", node, options=["--stream", "--external-dirs={0}".format(external_dir)])

        # Delta backup without external directory
        self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--external-dirs=none", "--stream"])
    ...
    def test_external_dirs_overlapping(self):
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: External directory path (-E option)" in e.message
                and "contain another external directory" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_external_dir_mapping(self):
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: --external-mapping option" in e.message
                and "have an entry in list of external directories" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        self.backup_node(
            ...
            options=[
                ...
                "--stream",
                "-E",
                "{0}{1}{2}".format(external_dir1, self.EXTERNAL_DIRECTORY_DELIMITER, external_dir2),
            ],
        )
        ...
        pgdata_restored = self.pgdata_content(node_restored.base_dir, exclude_dirs=["logs"])
        self.compare_pgdata(pgdata, pgdata_restored)
    ...
    def test_backup_multiple_external(self):
        ...
        self.set_config(backup_dir, "node", options=["-E", external_dir1])

        # cmdline option MUST override options in config
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream", "-E", external_dir2]
        )

        pgdata = self.pgdata_content(node.base_dir, exclude_dirs=["logs", "external_dir1"])
    ...
    def test_external_backward_compatibility(self):
        ...
        self.backup_node(backup_dir, "node", node, old_binary=True, options=["-j", "4", "--stream"])
        ...
        # FULL backup
        backup_id = self.backup_node(backup_dir, "node", node, old_binary=True, options=["-j", "4", "--stream"])
    ...
    def test_external_backward_compatibility_merge_1(self):
        ...
        tmp_id = self.backup_node(backup_dir, "node", node, old_binary=True, options=["-j", "4", "--stream"])
    ...
    def test_external_backward_compatibility_merge_2(self):
        ...
        self.restore_node(backup_dir, "node", node, data_dir=external_dir1, options=["-j", "4", "--skip-external-dirs"])
    ...
    def test_external_merge(self):
        ...
        tmp_id = self.backup_node(backup_dir, "node", node, options=["-j", "4", "--stream"])

        external_dir1 = self.get_tblspace_path(node, "external_dir1")
        external_dir2 = self.get_tblspace_path(node, "external_dir2")

        # fill external directories with data
        self.restore_node(backup_dir, "node", node, backup_id=tmp_id, data_dir=external_dir1, options=["-j", "4"])

        self.restore_node(backup_dir, "node", node, backup_id=tmp_id, data_dir=external_dir2, options=["-j", "4"])
        ...
        # Merge
        print(self.merge_backup(backup_dir, "node", backup_id=backup_id, options=["--log-level-file=VERBOSE"]))
    ...
    def test_external_merge_1(self):
        ...
        # FULL backup
        self.backup_node(backup_dir, "node", node, options=["-j", "4", "--stream"])
        ...
        backup_id = self.backup_node(backup_dir, "node", node, options=["-j", "4", "--stream"])
    ...
    def test_external_merge_3(self):
        ...
        # fill external directories with changed data
        self.restore_node(backup_dir, "node", node, data_dir=external_dir1)

        self.restore_node(backup_dir, "node", node, data_dir=external_dir2)
        ...
        self.merge_backup(backup_dir, "node", backup_id=backup_id, options=["--log-level-file=verbose"])
    ...
    def test_restore_external_changed_data_1(self):
        ...
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream", "-E", external_dir1]
        )

        pgdata = self.pgdata_content(node.base_dir, exclude_dirs=["logs", "external_dir2"])
        ...
        output = self.restore_node(backup_dir, "node", node, options=["-j", "4"])

        self.assertNotIn("externaldir2", output)

        pgdata_restored = self.pgdata_content(node.base_dir, exclude_dirs=["logs", "external_dir2"])
    ...
    def test_restore_skip_external(self):
        ...
        self.restore_node(backup_dir, "node", node, options=["-j", "4", "--skip-external-dirs"])
    ...
    def test_external_dir_is_symlink(self):
        ...
        self.restore_node(backup_dir, "node", node, data_dir=symlinked_dir, options=["-j", "4"])
        ...
        external_dir_new = self.get_tblspace_path(node_restored, "external_dir")

        self.restore_node(
            backup_dir,
            "node",
            node_restored,
            options=["-j", "4", "--external-mapping={0}={1}".format(external_dir, external_dir_new)],
        )
        ...
        self.assertEqual(external_dir, self.show_pb(backup_dir, "node", backup_id=backup_id)["external-dirs"])
    ...
    def test_external_dir_is_tablespace(self):
        ...
        self.create_tblspace_in_node(node, "tblspace1", tblspc_path=external_dir)

        node.pgbench_init(scale=1, tablespace="tblspace1")

        # FULL backup with external directories
        try:
            self.backup_node(backup_dir, "node", node, options=["-j", "4", "--stream", "-E", external_dir])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_restore_external_dir_not_empty(self):
        ...
        try:
            self.restore_node(
                backup_dir, "node", node, options=["--external-mapping={0}={1}".format(external_dir, external_dir_new)]
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
    ...
    def test_restore_external_dir_is_missing(self):
        ...
        try:
            self.backup_node(
                backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream", "-E", external_dir]
            )
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
    ...
    def test_merge_external_dir_is_missing(self):
        ...
        # take DELTA without external directories
        backup_id = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["-j", "4", "--stream"])
    ...
    def test_restore_external_dir_string_order(self):
        ...
        self.backup_node(
            ...
            options=[
                ...
                "--stream",
                "-E",
                "{0}{1}{2}".format(external_dir_1, self.EXTERNAL_DIRECTORY_DELIMITER, external_dir_2),
            ],
        )
        ...
        self.backup_node(
            ...
            options=[
                ...
                "--stream",
                "-E",
                "{0}{1}{2}".format(external_dir_2, self.EXTERNAL_DIRECTORY_DELIMITER, external_dir_1),
            ],
        )
    ...
    def test_smart_restore_externals(self):
        ...
        self.restore_node(backup_dir, "node", node, backup_id=tmp_id, data_dir=external_dir_1, options=["-j", "4"])

        self.restore_node(backup_dir, "node", node, backup_id=tmp_id, data_dir=external_dir_2, options=["-j", "4"])
    ...
    def test_external_validation(self):
        ...
        # take temp FULL backup
        tmp_id = self.backup_node(backup_dir, "node", node, options=["--stream"])

        external_dir = self.get_tblspace_path(node, "external_dir")

        # fill external directories with data
        self.restore_node(backup_dir, "node", node, backup_id=tmp_id, data_dir=external_dir, options=["-j", "4"])

        self.delete_pb(backup_dir, "node", backup_id=tmp_id)

        # take FULL backup
        full_id = self.backup_node(backup_dir, "node", node, options=["--stream", "-E", "{0}".format(external_dir)])

        # Corrupt file
        file = os.path.join(
            backup_dir, "backups", "node", full_id, "external_directories", "externaldir1", "postgresql.auto.conf"
        )
