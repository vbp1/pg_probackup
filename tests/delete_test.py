...
class DeleteTest(ProbackupTest, unittest.TestCase):
    ...
    def test_delete_archive_mix_compress_and_non_compressed_segments(self):
        """delete full backups"""
        node = self.make_simple_node(
            base_dir="{0}/{1}/node".format(self.module_name, self.fname), initdb_params=["--data-checksums"]
        )
        ...
        self.backup_node(backup_dir, "node", node, options=["--retention-redundancy=3", "--delete-expired"])
    ...
    def test_delete_orphaned_wal_segments(self):
        ...
        # delete useless wals
        self.assertTrue("On timeline 1 WAL segments between " in result and "will be removed" in result)
        ...
        wals = [f for f in os.listdir(wals_dir) if os.path.isfile(os.path.join(wals_dir, f))]
        self.assertTrue(
            original_wal_quantity > len(wals), "Number of wals not changed after 'delete --wal' which is illegal"
        )
    ...
    def test_delete_wal_between_multiple_timelines(self):
        ...
        self.backup_node(backup_dir, "node", node2, data_dir=node2.data_dir)
    ...
    def test_delete_backup_with_empty_control_file(self):
        ...
        with open(os.path.join(backup_dir, "backups", "node", backup_id, "backup.control"), "w") as f:
            f.flush()
            f.close()
    ...
    def test_delete_interleaved_incremental_chains(self):
        ...
        # Delete FULLb
        self.delete_pb(backup_dir, "node", backup_id_b)
    ...
    def test_delete_multiple_descendants_dry_run(self):
        ...
        output = self.delete_pb(
            backup_dir, "node", page_id_a1, options=["--dry-run", "--log-level-console=LOG", "--delete-wal"]
        )

        print(output)
        self.assertIn("LOG: Backup {0} can be deleted".format(page_id_a3), output)
        self.assertIn("LOG: Backup {0} can be deleted".format(page_id_a2), output)
        self.assertIn("LOG: Backup {0} can be deleted".format(page_id_a1), output)

        self.assertIn("INFO: Resident data size to free by delete of backup {0} :".format(page_id_a1), output)

        self.assertIn(
            "On timeline 1 WAL segments between 000000010000000000000001 and 000000010000000000000003 can be removed",
            output,
        )

        self.assertEqual(len(self.show_pb(backup_dir, "node")), 4)

        output = self.delete_pb(backup_dir, "node", page_id_a1, options=["--log-level-console=LOG", "--delete-wal"])

        self.assertIn("LOG: Backup {0} will be deleted".format(page_id_a3), output)
        self.assertIn("LOG: Backup {0} will be deleted".format(page_id_a2), output)
        self.assertIn("LOG: Backup {0} will be deleted".format(page_id_a1), output)
        self.assertIn("INFO: Resident data size to free by delete of backup {0} :".format(page_id_a1), output)

        self.assertIn(
            "On timeline 1 WAL segments between 000000010000000000000001 and 000000010000000000000003 will be removed",
            output,
        )
    ...
    def test_delete_error_backups(self):
        ...
        self.assertIn("Deleting all backups with status 'ERROR' in dry run mode", output)

        self.assertIn("INFO: Backup {0} with status OK can be deleted".format(backup_id_d), output)
