...
class ArchiveTest(ProbackupTest, unittest.TestCase):
    ...
    def test_pgpro434_1(self):
        ...
        self.assertEqual(result, node.table_checksum("t_heap"), "data after restore not equal to original data")
    ...
    def test_pgpro434_2(self):
        ...
        if self.get_version(node) < self.version_to_num("9.6.0"):
            self.skipTest("Skipped because pg_control_checkpoint() is not supported in PG 9.5")
        ...
        # SECOND TIMELIN
        node.cleanup()
        self.restore_node(backup_dir, "node", node, options=["--immediate", "--recovery-target-action=promote"])
        node.slow_start()

        if self.verbose:
            print(node.safe_psql("postgres", "select redo_wal_file from pg_control_checkpoint()"))
            self.assertFalse(
                node.execute("postgres", "select exists(select 1 from t_heap where id = 100501)")[0][0],
        ...
        self.assertFalse(
            node.execute("postgres", "select exists(select 1 from t_heap where id > 100500)")[0][0],
            "data after restore not equal to original data",
        )
    ...
    def test_pgpro434_3(self):
        ...
        gdb = self.backup_node(
            backup_dir, "node", node, options=["--archive-timeout=60", "--log-level-file=LOG"], gdb=True
        )
        ...
        if self.get_version(node) < 100000:
            self.assertIn("ERROR: pg_stop_backup doesn't answer in 60 seconds, cancel it", log_content)
        else:
            self.assertIn(
                "ERROR: WAL segment 000000010000000000000003 could not be archived in 60 seconds", log_content
            )
    ...
    def test_pgpro434_4(self):
        ...
        gdb = self.backup_node(
            backup_dir, "node", node, options=["--archive-timeout=60", "--log-level-file=info"], gdb=True
        )
        ...
        pid = (
            node.safe_psql("postgres", "SELECT pid FROM pg_stat_activity WHERE application_name = 'pg_probackup'")
            .decode("utf-8")
            .rstrip()
        )
        ...
        if self.get_version(node) < 150000:
            ...
            self.assertIn("ERROR: pg_backup_stop doesn't answer in 60 seconds, cancel it", log_content)
    ...
    def test_archive_push_file_exists(self):
        ...
        log = tail_file(log_file, linetimeout=30, totaltimeout=120, collect=True)
        log.wait(contains="The failed archive command was")

        self.assertIn("LOG:  archive command failed with exit code 1", log.content)

        self.assertIn("DETAIL:  The failed archive command was:", log.content)

        self.assertIn("pg_probackup archive-push WAL file", log.content)

        self.assertIn("WAL file already exists in archive with different checksum", log.content)

        self.assertNotIn("pg_probackup archive-push completed successfully", log.content)

        # btw check that console coloring codes are not slipped into log file
        self.assertNotIn("[0m", log.content)

        if self.get_version(node) < 100000:
            wal_src = os.path.join(node.data_dir, "pg_xlog", "000000010000000000000001")
        else:
    ...
    def test_archive_push_file_exists_overwrite(self):
        ...
        self.assertIn("DETAIL:  The failed archive command was:", log.content)
        self.assertIn("pg_probackup archive-push WAL file", log.content)
        self.assertNotIn("WAL file already exists in archive with different checksum, overwriting", log.content)
        self.assertIn("WAL file already exists in archive with different checksum", log.content)
        ...
        self.assertIn("WAL file already exists in archive with different checksum, overwriting", log.content)
    ...
    def test_archive_push_partial_file_exists(self):
        ...
        self.init_pb(backup_dir)
        self.add_instance(backup_dir, "node", node)
        self.set_archiving(backup_dir, "node", node, log_level="verbose", archive_timeout=60)
        ...
        xid = node.safe_psql("postgres", "INSERT INTO t1 VALUES (1) RETURNING (xmin)").decode("utf-8").rstrip()

        if self.get_version(node) < 100000:
            filename_orig = node.safe_psql(
                "postgres", "SELECT file_name FROM pg_xlogfile_name_offset(pg_current_xlog_location());"
            ).rstrip()
        else:
            filename_orig = node.safe_psql(
                "postgres", "SELECT file_name FROM pg_walfile_name_offset(pg_current_wal_flush_lsn());"
            ).rstrip()
        ...
        self.validate_pb(backup_dir, "node", options=["--recovery-target-xid={0}".format(xid)])

        log_file = os.path.join(node.logs_dir, "postgresql.log")
        with open(log_file) as f:
            log_content = f.read()

            self.assertIn("Reusing stale temp WAL file", log_content)
    ...
    def test_archive_push_part_file_exists_not_stale(self):
        ...
        node.safe_psql("postgres", "create table t2()")
    ...
    def test_replica_archive(self):
        ...
        master = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"archive_timeout": "10s", "checkpoint_timeout": "30s", "max_wal_size": "32MB"},
        )

        if self.get_version(master) < self.version_to_num("9.6.0"):
            self.skipTest("Skipped because backup from replica is not supported in PG 9.5")
    ...
    def test_basic_master_and_replica_concurrent_archiving(self):
        ...
        master = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "30s", "archive_timeout": "10s"},
        )
    ...
    def test_concurrent_archiving(self):
        ...
        replica1 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "replica1"))
        replica1.cleanup()
        ...
        pgbench = master.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-T", "30", "-c", "1"])
    ...
    def test_archive_pg_receivexlog(self):
        ...
        pg_receivexlog = self.run_binary(
            [pg_receivexlog_path, "-p", str(node.port), "--synchronous", "-D", os.path.join(backup_dir, "wal", "node")],
            asynchronous=True,
        )
    ...
    def test_archive_catalog(self):
        ...
        self.backup_node(backup_dir, "master", master, backup_type="page")
        ...
        # FULL backup replica
        self.backup_node(backup_dir, "replica", replica, options=["--stream", "--archive-timeout=60s"])

        master.pgbench_init(scale=5)

        # PAGE backup replica
        Y2 = self.backup_node(
            backup_dir, "replica", replica, backup_type="page", options=["--stream", "--archive-timeout=60s"]
        )
        ...
        # FULL backup replica
        A1 = self.backup_node(backup_dir, "replica", replica)

        replica.pgbench_init(scale=5)

        replica.safe_psql("postgres", "CREATE TABLE t1 (a text)")

        target_xid = None
        with replica.connect("postgres") as con:
            res = con.execute("INSERT INTO t1 VALUES ('inserted') RETURNING (xmin)")
            con.commit()
            target_xid = res[0][0]

        # DELTA backup replica
        self.backup_node(backup_dir, "replica", replica, backup_type="delta")
        ...
        B2 = self.backup_node(backup_dir, "replica", replica, backup_type="page")
        ...
        replica.safe_psql(
            "postgres",
            "CREATE TABLE t2 as select i, repeat(md5(i::text),5006056) as fat_attr from generate_series(0,6) i",
        )
        ...
        replica.safe_psql(
            "postgres",
            "CREATE TABLE t3 as select i, repeat(md5(i::text),5006056) as fat_attr from generate_series(0,10) i",
        )
        ...
        replica.safe_psql(
            "postgres",
            "CREATE TABLE t4 as select i, repeat(md5(i::text),5006056) as fat_attr from generate_series(0,6) i",
        )
        ...
        wals = [
            f
            for f in os.listdir(wals_dir)
            if os.path.isfile(os.path.join(wals_dir, f))
            and not f.endswith(".backup")
            and not f.endswith(".history")
            and f.startswith("00000003")
        ]
        wals.sort()
        ...
        self.assertEqual(timeline_3["lost-segments"][0]["begin-segno"], "000000030000000000000012")
        self.assertEqual(timeline_3["lost-segments"][0]["end-segno"], "000000030000000000000013")
        self.assertEqual(timeline_3["lost-segments"][1]["begin-segno"], "000000030000000000000017")
        self.assertEqual(timeline_3["lost-segments"][1]["end-segno"], "000000030000000000000017")
    ...
    def test_archive_catalog_1(self):
        ...
        if not self.archive_compress:
            self.skipTest("You need to enable ARCHIVE_COMPRESSION for this test to run")
        ...
        os.rename(
            os.path.join(wals_dir, "000000010000000000000001"), os.path.join(wals_dir, "000000010000000000000002")
        )
        ...
        for timeline in timelines:
            self.assertEqual(timeline["min-segno"], "000000010000000000000001")
    ...
    def test_archive_catalog_2(self):
        ...
        # sanity
        for timeline in timelines:
            self.assertEqual(timeline["min-segno"], "000000010000000000000002")
            self.assertEqual(timeline["status"], "OK")
    ...
    def test_archive_options(self):
        ...
        self.assertIn("restore_command = '\"cp {0}/%f %p\"'".format(wal_dir), recovery_content)
        ...
        self.restore_node(
            ...
            "node",
            node,
            options=["--archive-host=localhost", "--archive-port=22", "--archive-user={0}".format(self.user)],
        )
        ...
        self.assertIn(
            ...
            "--remote-port=22 --remote-user={3}'".format(self.probackup_path, backup_dir, "node", self.user),
            recovery_content,
        )
    ...
    def test_archive_options_1(self):
        ...
        self.assertIn(
            ...
            "--remote-port=23 --remote-user={3}'".format(self.probackup_path, backup_dir, "node", self.user),
            recovery_content,
        )
    ...
    def test_undefined_wal_file_path(self):
        ...
        self.set_auto_conf(node, {"archive_command": archive_command})

        node.slow_start()
        node.safe_psql("postgres", "create table t_heap as select i as id from generate_series(0, 10) i")
        self.switch_wal_segment(node)
    ...
    def test_intermediate_archiving(self):
        ...
        if os.name == "posix":
            ...
            self.set_archiving(
                ...
                "node",
                node,
                custom_archive_command='copy /Y "%p" "{0}\\\\%f"'.format(wal_dir.replace("\\", "\\\\")),
            )
        else:
            self.assertTrue(False, "Unexpected os family")
        ...
        self.run_pb(
            [
                ...
                node.data_dir,
                "--wal-file-path",
                "{0}/{1}".format(wal_dir, wal_segment),
                "--wal-file-name",
                wal_segment,
            ]
        )
    ...
    def test_waldir_outside_pgdata_archiving(self):
        ...
        if self.pg_config_version < self.version_to_num("10.0"):
            self.skipTest("Skipped because waldir outside pgdata is supported since PG 10")
    ...
    def test_hexadecimal_timeline(self):
        ...
        for i in range(1, 13):
            # print(i)
            node.cleanup()
            self.restore_node(backup_dir, "node", node, options=["--recovery-target-timeline={0}".format(i)])
            node.slow_start()
            node.pgbench_init(scale=2)
        ...
        self.assertEqual(13, tli13["tli"])

        self.assertEqual(12, tli13["parent-tli"])

        self.assertEqual(backup_id, tli13["closest-backup-id"])

        self.assertEqual("0000000D000000000000001C", tli13["max-segno"])
    ...
    def test_archiving_and_slots(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "30s", "max_wal_size": "64MB"},
        )
    ...
    def test_archive_push_sanity(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"archive_mode": "on", "archive_command": "exit 1"},
        )
        ...
        self.restore_node(backup_dir, "node", replica, data_dir=replica.data_dir, options=["-R"])
        ...
        log = tail_file(os.path.join(replica.logs_dir, "postgresql.log"), collect=True)
        ...
        output = self.show_archive(
            backup_dir, "node", as_json=False, as_text=True, options=["--log-level-console=INFO"]
        )
    ...
    def test_multi_timeline_recovery_prefetching(self):
        ...
        target_xid = node.safe_psql("postgres", "select txid_current()").rstrip()
        ...
        if node.major_version >= 12:
            node.append_conf("postgresql.auto.conf", "restore_command = '{0}'".format(restore_command))
        else:
            node.append_conf("recovery.conf", "restore_command = '{0}'".format(restore_command))
        ...
        self.assertIn(
            "pg_probackup archive-get failed to deliver WAL file: 000000030000000000000006", postgres_log_content
        )

        self.assertIn(
            "pg_probackup archive-get failed to deliver WAL file: 000000020000000000000006", postgres_log_content
        )

        self.assertIn(
            "pg_probackup archive-get used prefetched WAL segment 000000010000000000000006, prefetch state: 5/10",
    ...
    def test_archive_get_batching_sanity(self):
        ...
        self.restore_node(backup_dir, "node", replica, replica.data_dir)
        ...
        if node.major_version >= 12:
            ...
            replica.append_conf("recovery.conf", "restore_command = '{0}'".format(restore_command))
        ...
        self.assertIn("pg_probackup archive-get completed successfully, fetched: 10/10", postgres_log_content)
    ...
    def test_archive_show_partial_files_handling(self):
        ...
        self.wait_instance_wal_exists(backup_dir, "node", f"{filename}.gz")

        os.rename(os.path.join(wals_dir, filename), os.path.join(wals_dir, "{0}.part".format(filename)))
        ...
        os.rename(os.path.join(wals_dir, filename), os.path.join(wals_dir, "{0}.gz.part".format(filename)))

        # .partial file
        node.safe_psql("postgres", "create table t3()")
        ...
        os.rename(os.path.join(wals_dir, filename), os.path.join(wals_dir, "{0}.partial".format(filename)))

        # .gz.partial file
        node.safe_psql("postgres", "create table t4()")
        ...
        os.rename(os.path.join(wals_dir, filename), os.path.join(wals_dir, "{0}.gz.partial".format(filename)))
    ...
    def test_archive_empty_history_file(self):
        ...
        self.restore_node(
            ...
            "node",
            node,
            options=["--recovery-target=latest", "--recovery-target-timeline=2", "--recovery-target-action=promote"],
        )
        ...
        self.restore_node(
            ...
            "node",
            node,
            options=["--recovery-target=latest", "--recovery-target-timeline=3", "--recovery-target-action=promote"],
        )
        ...
        for tli in range(2, 5):
            file = os.path.join(backup_dir, "wal", "node", "0000000{0}.history".format(tli))
...
def cleanup_ptrack(log_content):
    ...
    if ptrack_is_not in log_content:
        lines = [line for line in log_content.splitlines() if ptrack_is_not not in line]
