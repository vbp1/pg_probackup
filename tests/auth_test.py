...
class SimpleAuthTest(ProbackupTest, unittest.TestCase):
    ...
    def test_backup_via_unprivileged_user(self):
        ...
        try:
            self.backup_node(backup_dir, "node", node, options=["-U", "backup"])
            self.assertEqual(1, 0, "Expecting Error due to missing grant on EXECUTE.")
        ...
        if self.get_version(node) < 150000:
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_start_backup(text, boolean, boolean) TO backup;")
        else:
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_backup_start(text, boolean) TO backup;")

        if self.get_version(node) < 100000:
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_catalog.pg_switch_xlog() TO backup")
        else:
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_catalog.pg_switch_wal() TO backup")
        ...
        node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_create_restore_point(text) TO backup;")
        ...
        try:
            ...
            if self.get_version(node) < 150000:
                self.assertIn(
                    "ERROR: Query failed: ERROR:  permission denied for function pg_stop_backup",
                    e.message,
                    "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
                )
            else:
                self.assertIn(
                    "ERROR: Query failed: ERROR:  permission denied for function pg_backup_stop",
                    e.message,
                    "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
                )

        if self.get_version(node) < self.version_to_num("10.0"):
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_stop_backup(boolean) TO backup")
            ...
            node.safe_psql("postgres", "GRANT EXECUTE ON FUNCTION pg_backup_stop(boolean) TO backup;")
        ...
        node.safe_psql("postgres", "ALTER ROLE backup REPLICATION")
        ...
        # PTRACK
        if self.ptrack:
            self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["-U", "backup"])
...
class AuthTest(unittest.TestCase):
    ...
    def tearDown(self):
        if self.pgpass_file_lock and hasattr(self, "pgpass_line") and os.path.exists(self.pgpass_file):
    ...
    def test_empty_password(self):
        ...
        try:
            self.assertIn("ERROR: no password supplied", self.run_pb_with_auth("\0\r\n"))
        except (TIMEOUT, ExceptionPexpect) as e:
            self.fail(e.value)

    def test_wrong_password(self):
        """Test case: PGPB_AUTH04 - incorrect password"""
        self.assertIn("password authentication failed", self.run_pb_with_auth("wrong_password\r\n"))

    def test_right_password(self):
        """Test case: PGPB_AUTH01 - correct password"""
        self.assertIn("completed", self.run_pb_with_auth("password\r\n"))
    ...
    def run_pb_with_auth(self, password=None, add_args=None, kill=False):
        if add_args is None:
            add_args = []
