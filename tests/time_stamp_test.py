...
class TimeStamp(ProbackupTest, unittest.TestCase):
    ...
    def test_server_date_style(self):
        ...
        self.backup_node(backup_dir, "node", node, options=["--stream", "-j 2"])
    ...
    def test_handling_of_TZ_env_variable(self):
        ...
        self.backup_node(backup_dir, "node", node, options=["--stream", "-j 2"], env=my_env)
    ...
    def test_dst_timezone_handling(self):
        ...
        subprocess.Popen(
            ["sudo", "timedatectl", "set-ntp", "false"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()

        subprocess.Popen(
            ["sudo", "timedatectl", "set-time", "2020-05-25 12:00:00"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
        ...
        # move to dst
        subprocess.Popen(
            ["sudo", "timedatectl", "set-time", "2020-10-25 12:00:00"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()

        # DELTA
        output = self.backup_node(backup_dir, "node", node, backup_type="delta", return_id=False)
        self.assertNotIn("backup ID in control file", output)

        subprocess.Popen(
            ["sudo", "timedatectl", "set-time", "2020-12-01 12:00:00"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
        ...
        subprocess.Popen(
            ["sudo", "timedatectl", "set-ntp", "true"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
        ...
        subprocess.Popen(
            ["sudo", "timedatectl", "set-timezone", "US/Moscow"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
    ...
    def test_dst_timezone_handling_backward_compatibilty(self):
        ...
        subprocess.Popen(
            ["sudo", "timedatectl", "set-timezone", "America/Detroit"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
        ...
        # DELTA
        output = self.backup_node(backup_dir, "node", node, backup_type="delta", old_binary=True, return_id=False)
