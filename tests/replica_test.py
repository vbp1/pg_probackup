...
class ReplicaTest(ProbackupTest, unittest.TestCase):
    ...
    def test_replica_switchover(self):
        ...
        self.backup_node(backup_dir, "node2", node2, backup_type="delta", options=["--stream"])
    ...
    def test_replica_archive_page_backup(self):
        ...
        self.assertEqual(before, after, "Restored data is not equal to original")
    ...
    def test_basic_make_replica_via_restore(self):
        ...
        master.table_checksum("t_heap")

        self.backup_node(backup_dir, "master", master, backup_type="page")
        self.restore_node(backup_dir, "master", replica, options=["-R"])
        ...
        self.backup_node(backup_dir, "replica", replica, options=["--archive-timeout=30s", "--stream"])
    ...
    def test_take_backup_from_delayed_replica(self):
        ...
        if self.get_version(master) >= self.version_to_num("12.0"):
            ...
            replica.append_conf("recovery.conf", "recovery_min_apply_delay = 300s")
        ...
        pgbench = master.pgbench(options=["-T", "60", "-c", "2", "--no-vacuum"])

        self.backup_node(backup_dir, "replica", replica, options=["--archive-timeout=60s"])
        ...
        self.backup_node(
            ...
            backup_type="page",
            options=["--archive-timeout=60s"],
        )

        sleep(1)

        self.backup_node(backup_dir, "replica", replica, backup_type="delta", options=["--archive-timeout=60s"])
        ...
        self.backup_node(backup_dir, "replica", replica, backup_type="page", options=["--stream"])

        self.backup_node(backup_dir, "replica", replica, backup_type="delta", options=["--stream"])
    ...
    def test_replica_promote(self):
        ...
        self.add_instance(backup_dir, "replica", replica)
        self.set_archiving(backup_dir, "replica", replica, replica=True)
        self.set_replica(master, replica, replica_name="replica", synchronous=True)
        ...
        gdb = self.backup_node(backup_dir, "replica", replica, gdb=True, options=["--log-level-file=verbose"])
        ...
        self.assertIn("ERROR:  the standby was promoted during online backup", log_content)

        self.assertIn("WARNING: Backup {0} is running, setting its status to ERROR".format(backup_id), log_content)
    ...
    def test_replica_stop_lsn_null_offset(self):
        ...
        master = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "1h", "wal_level": "replica"},
        )
        ...
        output = self.backup_node(
            ...
            replica,
            replica.data_dir,
            options=["--archive-timeout=30", "--log-level-console=LOG", "--no-validate", "--stream"],
            return_id=False,
        )

        self.assertIn("LOG: Invalid offset in stop_lsn value 0/4000000", output)

        self.assertIn("WARNING: WAL segment 000000010000000000000004 could not be streamed in 30 seconds", output)

        self.assertIn("WARNING: Failed to get next WAL record after 0/4000000, looking for previous WAL record", output)

        self.assertIn("LOG: Looking for LSN 0/4000000 in segment: 000000010000000000000003", output)

        self.assertIn("has endpoint 0/4000000 which is equal or greater than requested LSN 0/4000000", output)

        self.assertIn("LOG: Found prior LSN:", output)
    ...
    def test_replica_stop_lsn_null_offset_next_record(self):
        ...
        # freeze bgwriter to get rid of RUNNING XACTS records
        master.auxiliary_pids[ProcessType.BackgroundWriter][0]
        ...
        gdb = self.backup_node(
            ...
            "replica",
            replica,
            options=["--archive-timeout=40", "--log-level-file=LOG", "--no-validate", "--stream"],
            gdb=True,
        )
        ...
        self.assertIn("LOG: Invalid offset in stop_lsn value 0/4000000", log_content)

        self.assertIn("LOG: Looking for segment: 000000010000000000000004", log_content)

        self.assertIn('LOG: First record in WAL segment "000000010000000000000004": 0/4000028', log_content)

        self.assertIn("INFO: stop_lsn: 0/4000000", log_content)
    ...
    def test_archive_replica_null_offset(self):
        ...
        output = self.backup_node(
            ...
            replica,
            replica.data_dir,
            options=["--archive-timeout=30", "--log-level-console=LOG", "--no-validate"],
            return_id=False,
        )

        self.assertIn("LOG: Invalid offset in stop_lsn value 0/4000000", output)

        self.assertIn("WARNING: WAL segment 000000010000000000000004 could not be archived in 30 seconds", output)
    ...
    def test_replica_toast(self):
        ...
        master = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "1h", "wal_level": "replica", "shared_buffers": "128MB"},
        )
        ...
        master.safe_psql(
            "postgres",
            "CREATE TABLE t1 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,10) i",
        )
        ...
        self.assertIn("WARNING: Could not read WAL record at", output)

        self.assertIn("LOG: Found prior LSN:", output)

        res1 = replica.safe_psql("postgres", "select md5(fat_attr) from t1")
        ...
        res2 = replica.safe_psql("postgres", "select md5(fat_attr) from t1")
    ...
    def test_replica_promote_1(self):
        ...
        wal_file = os.path.join(backup_dir, "wal", "master", "000000010000000000000004")

        wal_file_partial = os.path.join(backup_dir, "wal", "master", "000000010000000000000004.partial")
        ...
        self.assertTrue(os.path.exists(wal_file_partial), "File {0} disappeared".format(wal_file))

        self.assertTrue(os.path.exists(wal_file_partial), "File {0} disappeared".format(wal_file_partial))
    ...
    def test_replica_promote_2(self):
        ...
        master.safe_psql(
            "postgres",
            "CREATE TABLE t1 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,1) i",
        )
        ...
        self.backup_node(backup_dir, "master", replica, data_dir=replica.data_dir, backup_type="page")
    ...
    def test_replica_promote_archive_delta(self):
        ...
        node1 = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "30s", "archive_timeout": "30s"},
        )
        ...
        node1.safe_psql(
            "postgres",
            "CREATE TABLE t1 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,20) i",
        )
        self.wait_until_replica_catch_with_master(node1, node2)

        node1.safe_psql(
            "postgres",
            "CREATE TABLE t2 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,20) i",
        )
        self.wait_until_replica_catch_with_master(node1, node2)

        # delta backup on replica on timeline 1
        delta1_id = self.backup_node(backup_dir, "node", node2, node2.data_dir, "delta", options=["--stream"])

        # delta backup on replica on timeline 1
        delta2_id = self.backup_node(backup_dir, "node", node2, node2.data_dir, "delta")
        ...
        node2.safe_psql(
            "postgres",
            "CREATE TABLE t3 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,20) i",
        )
        ...
        self.restore_node(
            ...
            node1.data_dir,
            backup_id=delta1_id,
            options=["--recovery-target-timeline=2", "--recovery-target=latest"],
        )
        ...
        node2.safe_psql(
            "postgres",
            "CREATE TABLE t4 AS SELECT i, repeat(md5(i::text),5006056) AS fat_attr FROM generate_series(0,30) i",
        )
        ...
        self.backup_node(backup_dir, "node", node1, node1.data_dir, "delta", options=["--archive-timeout=60"])
    ...
    def test_replica_promote_archive_page(self):
        ...
        page1_id = self.backup_node(backup_dir, "node", node2, node2.data_dir, "page", options=["--stream"])

        # page backup on replica on timeline 1
        page2_id = self.backup_node(backup_dir, "node", node2, node2.data_dir, "page")
        ...
        self.backup_node(backup_dir, "node", node1, node1.data_dir, "page", options=["--archive-timeout=60"])
    ...
    def test_instance_from_the_past(self):
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: Current START LSN" in e.message
                and "is lower than START LSN" in e.message
                and "It may indicate that we are trying to backup PostgreSQL instance from the past" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_replica_via_basebackup(self):
        ...
        self.backup_node(backup_dir, "node", node, backup_type="full", options=["--stream", "--log-level-file=verbose"])
        ...
        self.assertTrue(os.path.exists(filepath), "History file do not exists: {0}".format(filepath))
