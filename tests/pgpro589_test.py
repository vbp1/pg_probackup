    ...
    def test_pgpro589(self):
        ...
        try:
            ...
            self.assertTrue(
                ...
                and "ERROR: WAL segment" in e.message
                and "could not be archived in 10 seconds" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        backup_id = self.show_pb(backup_dir, "node")[0]["id"]
        self.assertEqual(
            "ERROR", self.show_pb(backup_dir, "node", backup_id)["status"], "Backup should have ERROR status"
        )
