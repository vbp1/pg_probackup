...
class RestoreTest(ProbackupTest, unittest.TestCase):
    ...
    def test_restore_to_specific_timeline(self):
        ...
        pgbench = node.pgbench(
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-T", "10", "-c", "2", "--no-vacuum"]
        )
        pgbench.wait()
        pgbench.stdout.close()
    ...
    def test_restore_to_time(self):
        ...
        target_time = node.execute("postgres", "SELECT to_char(now(), 'YYYY-MM-DD HH24:MI:SS+00')")[0][0]
        pgbench = node.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        pgbench.wait()
        pgbench.stdout.close()
        ...
        self.assertIn(
            ...
            self.restore_node(
                ...
                "node",
                node,
                options=["-j", "4", "--time={0}".format(target_time), "--recovery-target-action=promote"],
            ),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
    ...
    def test_restore_to_xid_inclusive(self):
        ...
        self.assertIn(
            ...
            self.restore_node(
                ...
                "node",
                node,
                options=["-j", "4", "--xid={0}".format(target_xid), "--recovery-target-action=promote"],
            ),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
    ...
    def test_restore_to_lsn_inclusive(self):
        ...
        self.assertIn(
            ...
            self.restore_node(
                ...
                "node",
                node,
                options=["-j", "4", "--lsn={0}".format(target_lsn), "--recovery-target-action=promote"],
            ),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
        ...
        after = node.table_checksum("pgbench_branches")
        self.assertEqual(before, after)
        self.assertEqual(len(node.execute("postgres", "SELECT * FROM tbl0005")), 2)
    ...
    def test_restore_full_ptrack_under_load(self):
        ...
        pgbench = node.pgbench(stdout=subprocess.PIPE, stderr=subprocess.STDOUT, options=["-c", "4", "-T", "8"])
    ...
    def test_restore_with_tablespace_mapping_1(self):
        ...
        try:
            ...
            self.assertEqual(
                1,
                0,
                "Expecting Error because restore tablespace destination is not empty.\n Output: {0} \n CMD: {1}".format(
                    repr(self.output), self.cmd
                ),
            )
        except ProbackupException as e:
        ...
        try:
            self.restore_node(backup_dir, "node", node, options=["-T", "%s=%s" % (tblspc_path, tblspc_path_temp)])
            # we should die here because exception is what we expect to happen
            self.assertEqual(
                1,
                0,
    ...
    def test_restore_with_tablespace_mapping_2(self):
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[1]["backup-mode"], "PAGE")
        ...
        self.assertEqual(self.show_pb(backup_dir, "node")[2]["backup-mode"], "PAGE")
    ...
    def test_restore_with_missing_or_corrupted_tablespace_map(self):
        ...
        node2 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node2"))
        node2.cleanup()
        ...
        # drop tablespace_map
        tablespace_map = os.path.join(backup_dir, "backups", "node", page_id, "database", "tablespace_map")

        tablespace_map_tmp = os.path.join(backup_dir, "backups", "node", page_id, "database", "tablespace_map_tmp")

        os.rename(tablespace_map, tablespace_map_tmp)

        try:
            self.restore_node(backup_dir, "node", node2, options=["-T", "{0}={1}".format(olddir, newdir)])
            self.assertEqual(
                1,
                0,
            ...
            self.assertIn(
                'ERROR: Tablespace map is missing: "{0}", probably backup {1} is corrupt, validate it'.format(
        ...
        print(self.restore_node(backup_dir, "node", node2, options=["-T", "{0}={1}".format(olddir, newdir)]))
    ...
    def test_archive_node_backup_archive_pitr_2(self):
        ...
        node_restored = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored"))
    ...
    def test_archive_restore_to_restore_point(self):
        ...
        node.safe_psql("postgres", "create table t_heap as select generate_series(0,10000)")
        result = node.table_checksum("t_heap")
        node.safe_psql("postgres", "select pg_create_restore_point('savepoint')")
        node.safe_psql("postgres", "create table t_heap_1 as select generate_series(0,10000)")
        node.cleanup()

        self.restore_node(
            backup_dir, "node", node, options=["--recovery-target-name=savepoint", "--recovery-target-action=promote"]
        )
        ...
        self.assertEqual(res[0], 1, "Table t_heap_1 should not exist in restored instance")
    ...
    def test_zags_block_corrupt(self):
        ...
        with node.connect("postgres") as conn:
            ...
            conn.commit()
            conn.execute("create index idx ON tbl (i)")
            conn.commit()
            conn.execute("insert into tbl select i from generate_series(0,400) as i")
            conn.commit()
            conn.execute("select pg_relation_size('idx')")
            conn.commit()
            conn.execute("delete from tbl where i < 100")
            conn.commit()
            conn.execute("explain analyze select i from tbl order by i")
            conn.commit()
            conn.execute("select i from tbl order by i")
            conn.commit()
            conn.execute("create extension pageinspect")
            conn.commit()
            print(conn.execute("select * from bt_page_stats('idx',1)"))
            conn.commit()
            conn.execute("insert into tbl select i from generate_series(0,100) as i")
            conn.commit()
            conn.execute("insert into tbl select i from generate_series(0,100) as i")
            conn.commit()
            conn.execute("insert into tbl select i from generate_series(0,100) as i")
        ...
        node_restored = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node_restored"), initdb_params=["--data-checksums"]
        )
        ...
        self.set_auto_conf(node_restored, {"archive_mode": "off", "hot_standby": "on", "port": node_restored.port})
    ...
    def test_zags_block_corrupt_1(self):
        node = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node"),
            initdb_params=["--data-checksums"],
            pg_options={"full_page_writes": "on"},
        )
        backup_dir = os.path.join(self.tmp_path, self.module_name, self.fname, "backup")
        self.init_pb(backup_dir)
        self.add_instance(backup_dir, "node", node)
        ...
        node.safe_psql("postgres", "insert into tbl select i from generate_series(0,100000) as i")

        node.safe_psql("postgres", "delete from tbl where i%2 = 0")

        node.safe_psql("postgres", "explain analyze select i from tbl order by i")

        node.safe_psql("postgres", "select i from tbl order by i")

        node.safe_psql("postgres", "create extension pageinspect")

        node.safe_psql("postgres", "insert into tbl select i from generate_series(0,100) as i")
    ...
    def test_restore_chain(self):
        ...
        # Take ERROR DELTA
        try:
            self.backup_node(backup_dir, "node", node, backup_type="delta", options=["-U", "wrong_name"])
        ...
        self.assertEqual("OK", self.show_pb(backup_dir, "node")[0]["status"], 'Backup STATUS should be "OK"')

        self.assertEqual("OK", self.show_pb(backup_dir, "node")[1]["status"], 'Backup STATUS should be "OK"')

        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[2]["status"], 'Backup STATUS should be "ERROR"')

        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[3]["status"], 'Backup STATUS should be "ERROR"')

        self.assertEqual("OK", self.show_pb(backup_dir, "node")[4]["status"], 'Backup STATUS should be "OK"')

        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[5]["status"], 'Backup STATUS should be "ERROR"')
    ...
    def test_restore_chain_with_corrupted_backup(self):
        ...
        # Take ERROR DELTA
        try:
            self.backup_node(backup_dir, "node", node, backup_type="page", options=["-U", "wrong_name"])
        except ProbackupException:
            pass
        ...
        # Corrupted 4 DELTA
        corrupt_id = self.backup_node(backup_dir, "node", node, backup_type="delta")

        # ORPHAN 5 DELTA
        restore_target_id = self.backup_node(backup_dir, "node", node, backup_type="delta")
        ...
        file = os.path.join(backup_dir, "backups", "node", corrupt_id, "database", "global", "pg_control")
        ...
        try:
            self.restore_node(backup_dir, "node", node, backup_id=restore_target_id)
            self.assertEqual(
                1,
                0,
        ...
        self.assertEqual("OK", self.show_pb(backup_dir, "node")[3]["status"], 'Backup STATUS should be "OK"')

        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[4]["status"], 'Backup STATUS should be "ERROR"')

        self.assertEqual("OK", self.show_pb(backup_dir, "node")[5]["status"], 'Backup STATUS should be "OK"')

        self.assertEqual("ERROR", self.show_pb(backup_dir, "node")[6]["status"], 'Backup STATUS should be "ERROR"')

        self.assertEqual("OK", self.show_pb(backup_dir, "node")[7]["status"], 'Backup STATUS should be "OK"')

        # corruption victim
        self.assertEqual("CORRUPT", self.show_pb(backup_dir, "node")[8]["status"], 'Backup STATUS should be "CORRUPT"')

        # orphaned child
        self.assertEqual("ORPHAN", self.show_pb(backup_dir, "node")[9]["status"], 'Backup STATUS should be "ORPHAN"')

        # orphaned child
        self.assertEqual("ORPHAN", self.show_pb(backup_dir, "node")[10]["status"], 'Backup STATUS should be "ORPHAN"')

        # next FULL
        self.assertEqual("OK", self.show_pb(backup_dir, "node")[11]["status"], 'Backup STATUS should be "OK"')

        # next DELTA
        self.assertEqual("OK", self.show_pb(backup_dir, "node")[12]["status"], 'Backup STATUS should be "OK"')
    ...
    def test_restore_backup_from_future(self):
        ...
        with open(os.path.join(backup_dir, "backups", "node", backup_id, "backup.control"), "a") as conf:
            conf.write("start-time='{:%Y-%m-%d %H:%M:%S}'\n".format(datetime.now() + timedelta(days=3)))

        # rename directory
        new_id = self.show_pb(backup_dir, "node")[1]["id"]

        os.rename(
            os.path.join(backup_dir, "backups", "node", backup_id), os.path.join(backup_dir, "backups", "node", new_id)
        )
    ...
    def test_restore_target_immediate_stream(self):
        ...
        node.cleanup()
        self.restore_node(backup_dir, "node", node, options=["--immediate"])

        self.assertTrue(os.path.isfile(recovery_conf), "File {0} do not exists".format(recovery_conf))

        # restore delta backup
        node.cleanup()
        self.restore_node(backup_dir, "node", node, options=["--recovery-target=immediate"])
    ...
    def test_restore_target_new_options(self):
        ...
        node.safe_psql("postgres", "CREATE TABLE tbl0005 (a text)")
        ...
        self.assertIn("recovery_target_time = '{0}'".format(target_time), recovery_conf_content)

        self.assertIn("recovery_target_action = 'promote'", recovery_conf_content)

        self.assertIn("recovery_target_timeline = '1'", recovery_conf_content)
        ...
        self.assertIn("recovery_target_xid = '{0}'".format(target_xid), recovery_conf_content)
        ...
        self.assertIn("recovery_target_name = '{0}'".format(target_name), recovery_conf_content)
        ...
        if self.get_version(node) >= 100000:
            ...
            self.assertIn("recovery_target_lsn = '{0}'".format(target_lsn), recovery_conf_content)
    ...
    def test_smart_restore(self):
        ...
        # drop database
        node.safe_psql("postgres", "DROP DATABASE testdb")
        ...
        node.cleanup()
        self.restore_node(
            backup_dir, "node", node, backup_id=page_id, options=["--no-validate", "--log-level-file=VERBOSE"]
        )
        ...
        filelist_diff = self.get_backup_filelist_diff(filelist_full, filelist_page)
    ...
    def test_pg_11_group_access(self):
        ...
        node = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node"),
            set_replication=True,
            initdb_params=["--data-checksums", "--allow-group-access"],
        )
    ...
    def test_restore_concurrent_drop_table(self):
        ...
        gdb = self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--stream", "--compress", "--no-validate"], gdb=True
        )

        gdb.set_breakpoint("backup_data_file")
        gdb.run_until_break()

        node.safe_psql("postgres", "DROP TABLE pgbench_accounts")
    ...
    def test_lost_non_data_file(self):
        ...
        file = os.path.join(backup_dir, "backups", "node", backup_id, "database", "postgresql.auto.conf")
        ...
        try:
            ...
            self.assertIn(
                "ERROR: Backup files restoring failed",
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_partial_restore_exclude(self):
        ...
        for i in range(1, 10, 1):
            node.safe_psql("postgres", "CREATE database db{0}".format(i))

        db_list_raw = (
            node.safe_psql("postgres", "SELECT to_json(a) FROM (SELECT oid, datname FROM pg_database) a")
            .decode("utf-8")
            .rstrip()
        )
        ...
        # restore FULL backup
        node_restored_1 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored_1"))
        node_restored_1.cleanup()

        try:
            self.restore_node(backup_dir, "node", node_restored_1, options=["--db-include=db1", "--db-exclude=db2"])
            self.assertEqual(
                1,
                0,
            ...
            self.assertIn(
                "ERROR: You cannot specify '--db-include' and '--db-exclude' together",
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        self.restore_node(backup_dir, "node", node_restored_1)

        pgdata_restored_1 = self.pgdata_content(node_restored_1.data_dir)
        self.compare_pgdata(pgdata, pgdata_restored_1)

        db1_path = os.path.join(node_restored_1.data_dir, "base", db_list["db1"])
        db5_path = os.path.join(node_restored_1.data_dir, "base", db_list["db5"])

        self.truncate_every_file_in_dir(db1_path)
        self.truncate_every_file_in_dir(db5_path)
        pgdata_restored_1 = self.pgdata_content(node_restored_1.data_dir)

        node_restored_2 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored_2"))
        node_restored_2.cleanup()

        self.restore_node(backup_dir, "node", node_restored_2, options=["--db-exclude=db1", "--db-exclude=db5"])
        ...
        node_restored_2.safe_psql("postgres", "select 1")

        try:
            node_restored_2.safe_psql("db1", "select 1")
        except QueryException as e:
            self.assertIn("FATAL", e.message)

        try:
            node_restored_2.safe_psql("db5", "select 1")
        except QueryException as e:
            self.assertIn("FATAL", e.message)
    ...
    def test_partial_restore_exclude_tablespace(self):
        ...
        tbl_oid = (
            node.safe_psql("postgres", "SELECT oid FROM pg_tablespace WHERE spcname = 'somedata'")
            .decode("utf-8")
            .rstrip()
        )

        for i in range(1, 10, 1):
            node.safe_psql("postgres", "CREATE database db{0} tablespace somedata".format(i))
        ...
        self.restore_node(
            backup_dir, "node", node_restored_1, options=["-T", "{0}={1}".format(node_tablespace, node1_tablespace)]
        )
        ...
        for db in db_list:
            ...
            if db in ["db1", "db5"]:
                self.truncate_every_file_in_dir(
                    os.path.join(node_restored_1.data_dir, "pg_tblspc", tbl_oid, version_specific_dir, db_list[db])
                )
        ...
        self.restore_node(
            ...
            "node",
            node_restored_2,
            options=["--db-exclude=db1", "--db-exclude=db5", "-T", "{0}={1}".format(node_tablespace, node2_tablespace)],
        )
    ...
    def test_partial_restore_include(self):
        ...
        for db in db_list:
            ...
            if db in ["template0", "template1", "postgres", "db1", "db5"]:
                continue
            self.truncate_every_file_in_dir(os.path.join(node_restored_1.data_dir, "base", db_list[db]))
        ...
        self.restore_node(
            ...
            "node",
            node_restored_2,
            options=["--db-include=db1", "--db-include=db5", "--db-include=postgres"],
        )
        ...
        node_restored_2.safe_psql("template1", "select 1")

        try:
            node_restored_2.safe_psql("db2", "select 1")
        except QueryException as e:
            self.assertIn("FATAL", e.message)

        try:
            node_restored_2.safe_psql("db10", "select 1")
        except QueryException as e:
            self.assertIn("FATAL", e.message)
    ...
    def test_partial_restore_backward_compatibility_1(self):
        ...
        backup_id = self.backup_node(backup_dir, "node", node, old_binary=True, options=["--stream"])
        ...
        try:
            self.restore_node(backup_dir, "node", node_restored, options=["--db-exclude=db5"])
            self.assertEqual(
                1,
                0,
            ...
            self.assertIn(
                "ERROR: Backup {0} doesn't contain a database_map, partial restore is impossible".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        db_list_raw = node.safe_psql(
            "postgres", "SELECT to_json(a) FROM (SELECT oid, datname FROM pg_database) a"
        ).rstrip()
        db_list_splitted = db_list_raw.splitlines()
        db_list = {}
        for line in db_list_splitted:
        ...
        backup_id = self.backup_node(backup_dir, "node", node, backup_type="delta", options=["--stream"])

        # get etalon
        node_restored.cleanup()
        self.restore_node(backup_dir, "node", node_restored)
        self.truncate_every_file_in_dir(os.path.join(node_restored.data_dir, "base", db_list["db5"]))
        self.truncate_every_file_in_dir(os.path.join(node_restored.data_dir, "base", db_list["db14"]))
        pgdata_restored = self.pgdata_content(node_restored.data_dir)
        ...
        self.restore_node(backup_dir, "node", node_restored_1, options=["--db-exclude=db5", "--db-exclude=db14"])
    ...
    def test_empty_and_mangled_database_map(self):
        ...
        path = os.path.join(backup_dir, "backups", "node", backup_id, "database", "database_map")
        ...
        try:
            self.restore_node(backup_dir, "node", node_restored, options=["--db-include=db1", "--no-validate"])
            self.assertEqual(
                1,
                0,
            ...
            self.assertIn(
                "ERROR: Backup {0} has empty or mangled database_map, partial restore is impossible".format(backup_id),
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )

        try:
            self.restore_node(backup_dir, "node", node_restored, options=["--db-exclude=db1", "--no-validate"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because database_map is empty.\n Output: {0} \n CMD: {1}".format(
        ...
        try:
            ...
            self.assertIn(
                'ERROR: Field "dbOid" is not found in the line 42 of the file backup_content.control',
                e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
        ...
        self.restore_node(backup_dir, "node", node_restored, options=["--no-validate"])
    ...
    def test_missing_database_map(self):
        ...
        node.safe_psql("postgres", "CREATE DATABASE backupdb")
        ...
        try:
            self.restore_node(backup_dir, "node", node_restored, options=["--db-exclude=db5", "--db-exclude=db9"])
            self.assertEqual(
                1,
                0,
        ...
        try:
            self.restore_node(backup_dir, "node", node_restored, options=["--db-include=db1"])
            self.assertEqual(
                1,
                0,
                "Expecting Error because user do not have pg_database access.\n Output: {0} \n CMD: {1}".format(
    ...
    def test_stream_restore_command_option(self):
        ...
        node.safe_psql("postgres", "create table t1()")
        ...
        self.restore_node(backup_dir, "node", node, options=["--restore-command={0}".format(restore_cmd)])

        self.assertTrue(os.path.isfile(recovery_conf), "File '{0}' do not exists".format(recovery_conf))

        if self.get_version(node) >= self.version_to_num("12.0"):
            recovery_signal = os.path.join(node.data_dir, "recovery.signal")
            self.assertTrue(os.path.isfile(recovery_signal), "File '{0}' do not exists".format(recovery_signal))

        node.slow_start()

        node.safe_psql("postgres", "select * from t1")

        timeline_id = (
            node.safe_psql("postgres", "select timeline_id from pg_control_checkpoint()").decode("utf-8").rstrip()
        )
    ...
    def test_restore_primary_conninfo(self):
        ...
        replica = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "replica"))
        replica.cleanup()
        str_conninfo = "host=192.168.1.50 port=5432 user=foo password=foopass"

        self.restore_node(backup_dir, "node", replica, options=["-R", "--primary-conninfo={0}".format(str_conninfo)])

        if self.get_version(node) >= self.version_to_num("12.0"):
            standby_signal = os.path.join(replica.data_dir, "standby.signal")
            self.assertTrue(os.path.isfile(standby_signal), "File '{0}' do not exists".format(standby_signal))
    ...
    def test_restore_primary_slot_info(self):
        ...
        node.safe_psql("SELECT pg_create_physical_replication_slot('master_slot')")

        self.restore_node(backup_dir, "node", replica, options=["-R", "--primary-slot-name=master_slot"])
    ...
    def test_issue_249(self):
        ...
        node.safe_psql(
            "postgres", "CREATE TABLE t1 as SELECT * from pgbench_accounts where aid > 200000 and aid < 450000"
        )

        node.safe_psql("postgres", "DELETE from pgbench_accounts where aid > 200000 and aid < 450000")

        node.safe_psql("postgres", "select * from pgbench_accounts")

        # FULL backup
        self.backup_node(backup_dir, "node", node)

        node.safe_psql("postgres", "INSERT INTO pgbench_accounts SELECT * FROM t1")

        # restore FULL backup
        node_restored_1 = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "node_restored_1"))
        node_restored_1.cleanup()

        self.restore_node(backup_dir, "node", node_restored_1, options=["--db-include=db1"])

        self.set_auto_conf(node_restored_1, {"port": node_restored_1.port, "hot_standby": "off"})

        node_restored_1.slow_start()

        node_restored_1.safe_psql("db1", "select 1")

        try:
            node_restored_1.safe_psql("postgres", "select 1")
        except QueryException as e:
            self.assertIn("FATAL", e.message)
    ...
    def test_pg_12_probackup_recovery_conf_compatibility(self):
        ...
        time = node.safe_psql("SELECT current_timestamp(0)::timestamptz;").decode("utf-8").rstrip()
        ...
        self.restore_node(
            ...
            "node",
            node,
            options=["--recovery-target-time={0}".format(time), "--recovery-target-action=promote"],
            old_binary=True,
        )
        ...
        xid = node.safe_psql("SELECT txid_current()").decode("utf-8").rstrip()
        node.pgbench_init(scale=1)
        ...
        self.restore_node(
            ...
            "node",
            node,
            options=["--recovery-target-xid={0}".format(xid), "--recovery-target-action=promote"],
        )
    ...
    def test_drop_postgresql_auto_conf(self):
        ...
        self.restore_node(
            backup_dir, "node", node, options=["--recovery-target=latest", "--recovery-target-action=promote"]
        )
    ...
    def test_concurrent_restore(self):
        ...
        # DELTA backup
        self.backup_node(
            backup_dir, "node", node, backup_type="delta", options=["--stream", "--compress", "--no-validate"]
        )
        ...
        gdb = self.restore_node(backup_dir, "node", node, options=["--no-validate"], gdb=True)
    ...
    def test_restore_with_waldir(self):
        ...
        self.assertIn(
            "INFO: Restore of backup {0} completed.".format(backup_id),
            self.restore_node(backup_dir, "node", node, options=["-X", "%s" % (waldir_path)]),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
        node.slow_start()
    ...
    def test_restore_to_latest_timeline(self):
        ...
        restore_result = self.restore_node(
            backup_dir, "node", node, options=["-j", "4", "--recovery-target-timeline=latest", "-i", backup_id]
        )
        self.assertIn(
            "INFO: Restore of backup {0} completed.".format(backup_id),
            restore_result,
        ...
        node.cleanup()
        restore_result = self.restore_node(
            backup_dir, "node", node, options=["-j", "4", "--recovery-target=latest", "-i", backup_id]
        )
        self.assertIn(
            "INFO: Restore of backup {0} completed.".format(backup_id),
            restore_result,
        ...
        if self.pg_config_version >= self.version_to_num("12.0"):
            ...
            # restore from first backup
            restore_result = self.restore_node(backup_dir, "node", node, options=["-j", "4", "-i", backup_id])
    ...
    def test_restore_issue_313(self):
        ...
        """
        self._check_gdb_flag_or_skip_test()
        node = self.make_simple_node("node", set_replication=True, initdb_params=["--data-checksums"])
        ...
        for file in filelist:
            ...
            if (
                int(filelist[file]["is_datafile"]) == 0
                and not stat.S_ISDIR(int(filelist[file]["mode"]))
                and not filelist[file]["size"] == "0"
                and file != "database_map"
            ):
                count += 1
        ...
        for fname in os.listdir(node_restored.data_dir):
            if fname.endswith(".conf"):
                os.rename(os.path.join(node_restored.data_dir, fname), os.path.join(node.data_dir, fname))

        try:
            node.slow_start()
            # we should die here because exception is what we expect to happen
            self.assertEqual(1, 0, "Expecting Error because backup is not fully restored")
        ...
        with open(os.path.join(node.logs_dir, "postgresql.log")) as f:
            if self.pg_config_version >= 120000:
                self.assertIn('PANIC:  could not read file "global/pg_control"', f.read())
            else:
                self.assertIn("PANIC:  could not read from control file", f.read())
