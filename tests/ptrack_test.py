...
class PtrackTest(ProbackupTest, unittest.TestCase):
    ...
    def test_drop_rel_during_backup_ptrack(self):
        ...
        gdb = self.backup_node(
            backup_dir, "node", node, backup_type="ptrack", gdb=True, options=["--log-level-file=LOG"]
        )
    ...
    def test_ptrack_eat_my_data(self):
        ...
        # Logical comparison
        self.assertEqual(result, node.table_checksum("pgbench_accounts"), "Data loss")
    ...
    def test_ptrack_simple(self):
        ...
        node.safe_psql("postgres", "create table t_heap as select i as id from generate_series(0,1) i")
        ...
        # Logical comparison
        self.assertEqual(result, node_restored.table_checksum("t_heap"))
    ...
    def test_ptrack_unprivileged(self):
        ...
        node.safe_psql("backupdb", "CREATE SCHEMA ptrack")
        node.safe_psql("backupdb", "CREATE EXTENSION ptrack WITH SCHEMA ptrack")
        node.safe_psql("backupdb", "GRANT USAGE ON SCHEMA ptrack TO backup")

        node.safe_psql("backupdb", "GRANT SELECT ON TABLE pg_catalog.pg_extension TO backup")
        ...
        self.backup_node(
            backup_dir, "node", node, datname="backupdb", backup_type="ptrack", options=["--stream", "-U", "backup"]
        )
    ...
    def test_ptrack_enable(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "30s", "shared_preload_libraries": "ptrack"},
        )
    ...
    def test_ptrack_uncommitted_xact(self):
        ...
        con.execute("create table t_heap as select i as id from generate_series(0,1) i")
        ...
        self.restore_node(backup_dir, "node", node_restored, node_restored.data_dir, options=["-j", "4"])
    ...
    def test_ptrack_vacuum_full(self):
        ...
        node.safe_psql(
            "postgres", "create table t_heap tablespace somedata as select i as id from generate_series(0,1000000) i"
        )
        ...
        process = Thread(target=pg_connect.execute, args=["VACUUM FULL t_heap"])
        process.start()
    ...
    def test_ptrack_get_block(self):
        ...
        self.backup_node(backup_dir, "node", node, options=["--stream"])
        gdb = self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["--stream"], gdb=True)
    ...
    def test_ptrack_archive(self):
        ...
        self.assertIn(
            ...
            self.restore_node(
                ...
                options=["-j", "4", "--recovery-target-action=promote", "--time={0}".format(full_target_time)],
            ),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
        node.slow_start()
        ...
        self.assertIn(
            ...
            self.restore_node(
                ...
                options=["-j", "4", "--time={0}".format(ptrack_target_time), "--recovery-target-action=promote"],
            ),
            "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(self.output), self.cmd),
        )
    ...
    def test_ptrack_pgpro417(self):
        ...
        self.show_pb(backup_dir, "node", backup_id)["start-lsn"]
    ...
    def test_create_db_on_replica(self):
        ...
        self.restore_node(backup_dir, "replica", node_restored, backup_id=backup_id, options=["-j", "4"])
    ...
    def test_alter_table_set_tablespace_ptrack(self):
        ...
        node.safe_psql("postgres", "alter table t_heap set tablespace somedata_new")
        ...
        self.restore_node(
            ...
            options=[
                ...
                "-T",
                "{0}={1}".format(
                    self.get_tblspace_path(node, "somedata_new"), self.get_tblspace_path(node_restored, "somedata_new")
                ),
            ],
        )
    ...
    def test_alter_database_set_tablespace_ptrack(self):
        ...
        # ALTER DATABASE
        node.safe_psql("template1", "alter database postgres set tablespace somedata")
        ...
        node_restored = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node_restored"), port=node.port
        )
    ...
    def test_drop_tablespace(self):
        ...
        node.safe_psql("postgres", "alter table t_heap set tablespace somedata")
        # PTRACK BACKUP
        self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["--stream"])

        # Move table back to default tablespace
        node.safe_psql("postgres", "alter table t_heap set tablespace pg_default")
        # SECOND PTRACK BACKUP
        self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["--stream"])
        ...
        if self.paranoia:
            pgdata = self.pgdata_content(node.data_dir, ignore_ptrack=True)
        ...
        if self.paranoia:
            pgdata_restored = self.pgdata_content(node.data_dir, ignore_ptrack=True)

        node.slow_start()

        tblspc_exist = node.safe_psql(
            "postgres", "select exists(select 1 from pg_tablespace where spcname = 'somedata')"
        )
    ...
    def test_ptrack_alter_tablespace(self):
        ...
        restored_node = self.make_simple_node(base_dir=os.path.join(self.module_name, self.fname, "restored_node"))
        restored_node.cleanup()
        tblspc_path_new = self.get_tblspace_path(restored_node, "somedata_restored")
        self.restore_node(
            backup_dir, "node", restored_node, options=["-j", "4", "-T", "{0}={1}".format(tblspc_path, tblspc_path_new)]
        )
    ...
    def test_ptrack_multiple_segments(self):
        ...
        node = self.make_simple_node(
            ...
            ptrack_enable=True,
            initdb_params=["--data-checksums"],
            pg_options={"full_page_writes": "off"},
        )
        ...
        if node.major_version < 11:
            ...
            idx_ptrack["old_pages"] = self.get_md5_per_page_for_fork(idx_ptrack["path"], idx_ptrack["old_size"])
        ...
        if node.major_version < 11:
            ...
            if not self.check_ptrack_sanity(idx_ptrack):
                self.assertTrue(False, "Ptrack has failed to register changes in data files")
    ...
    def test_atexit_fail(self):
        ...
        try:
            self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["--stream", "-j 30"])
        ...
        self.assertEqual(node.safe_psql("postgres", "select * from pg_is_in_backup()").rstrip(), "f")
    ...
    def test_ptrack_clean(self):
        ...
        for i in idx_ptrack:
            if idx_ptrack[i]["type"] != "heap" and idx_ptrack[i]["type"] != "seq":
                node.safe_psql(
                    "postgres",
                    "create index {0} on {1} using {2}({3}) tablespace somedata".format(
        ...
        self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["-j10", "--stream"])
        ...
        self.backup_node(backup_dir, "node", node, backup_type="page", options=["-j10", "--stream"])
        node.safe_psql("postgres", "checkpoint")
    ...
    def test_ptrack_cluster_on_btree_replica(self):
        ...
        if master.major_version >= 11:
            master.safe_psql("postgres", "CREATE EXTENSION ptrack")
        ...
        self.backup_node(backup_dir, "replica", replica, backup_type="ptrack", options=["-j10", "--stream"])
    ...
    def test_ptrack_cluster_on_gist_replica(self):
        master = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "master"), set_replication=True, ptrack_enable=True
        )
    ...
    def test_ptrack_empty(self):
        ...
        # Take PTRACK backup
        backup_id = self.backup_node(backup_dir, "node", node, backup_type="ptrack", options=["-j10", "--stream"])
        ...
        self.restore_node(
            ...
            node_restored,
            backup_id=backup_id,
            options=["-j", "4", "-T{0}={1}".format(tblspace1, tblspace2)],
        )
    ...
    def test_basic_ptrack_truncate_replica(self):
        master = self.make_simple_node(
            ...
            ptrack_enable=True,
            initdb_params=["--data-checksums"],
            pg_options={"max_wal_size": "32MB", "archive_timeout": "10s", "checkpoint_timeout": "5min"},
        )
        ...
        if replica.major_version < 10:
            replica.safe_psql("postgres", "select pg_xlog_replay_pause()")
        else:
            replica.safe_psql("postgres", "select pg_wal_replay_pause()")
    ...
    def test_ptrack_vacuum_truncate_replica(self):
        ...
        self.backup_node(
            ...
            replica,
            backup_type="ptrack",
            options=["--stream", "--log-level-file=INFO", "--archive-timeout=30"],
        )
    ...
    def test_ptrack_recovery_1(self):
        node = self.make_simple_node(
            ...
            ptrack_enable=True,
            initdb_params=["--data-checksums"],
            pg_options={"shared_buffers": "512MB", "max_wal_size": "3GB"},
        )
    ...
    def test_ptrack_pg_resetxlog(self):
        ...
        self.run_binary([pg_resetxlog_path, "-D", node.data_dir, "-o 42", "-f"], asynchronous=False)
        ...
        try:
            ...
            self.assertTrue(
                "ERROR: LSN from ptrack_control " in e.message
                and "is greater than Start LSN of previous backup" in e.message,
                "\n Unexpected Error Message: {0}\n CMD: {1}".format(repr(e.message), self.cmd),
            )
    ...
    def test_corrupt_ptrack_map(self):
        ...
        if ptrack_version >= self.version_to_num("2.3"):
            ...
            self.assertIn('WARNING:  ptrack read map: incorrect checksum of file "{0}"'.format(ptrack_map), log_content)
            ...
            self.assertIn('FATAL:  ptrack init: incorrect checksum of file "{0}"'.format(ptrack_map), log_content)
    ...
    def test_horizon_lsn_ptrack(self):
        ...
        self.assertGreaterEqual(
            self.get_ptrack_version(node), self.version_to_num("2.1"), "You need ptrack >=2.1 for this test"
        )
        ...
        # FULL backup
        self.backup_node(backup_dir, "node", node, options=["--stream"], old_binary=True)
