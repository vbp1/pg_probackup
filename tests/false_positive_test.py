...
class FalsePositive(ProbackupTest, unittest.TestCase):
    ...
    def test_validate_wal_lost_segment(self):
        ...
        self.backup_node(backup_dir, "node", node)
        self.assertTrue("validation completed successfully" in self.validate_pb(backup_dir, "node"))
        ########
    ...
    def test_pg_10_waldir(self):
        ...
        node = self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node"),
            set_replication=True,
            initdb_params=["--data-checksums", "--waldir={0}".format(wal_dir)],
        )
        ...
        self.assertTrue(os.path.islink(os.path.join(node_restored.data_dir, "pg_wal")), "pg_wal should be symlink")
    ...
    def test_recovery_target_time_backup_victim(self):
        ...
        target_time = node.safe_psql("postgres", "select now()").rstrip()
        ...
        self.validate_pb(backup_dir, "node", options=["--recovery-target-time={0}".format(target_time)])
    ...
    def test_recovery_target_lsn_backup_victim(self):
        ...
        gdb = self.backup_node(backup_dir, "node", node, options=["--log-level-console=LOG"], gdb=True)
        ...
        self.validate_pb(backup_dir, "node", options=["--recovery-target-lsn={0}".format(target_lsn)])
    ...
    def test_streaming_timeout(self):
        ...
        node = self.make_simple_node(
            ...
            set_replication=True,
            initdb_params=["--data-checksums"],
            pg_options={"checkpoint_timeout": "1h", "wal_sender_timeout": "5s"},
        )
        ...
        self.assertIn("could not receive data from WAL stream", log_content)

        self.assertIn("ERROR: Problem in receivexlog", log_content)

    # @unittest.skip("skip")
    @unittest.expectedFailure
    def test_validate_all_empty_catalog(self):
        """ """
        self.make_simple_node(
            base_dir=os.path.join(self.module_name, self.fname, "node"), initdb_params=["--data-checksums"]
        )
